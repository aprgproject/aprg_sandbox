...
TEST(BreadthFirstSearchTest, HasPathToWorksWithDirectedGraph)
{
    GraphForTest graph;
...
TEST(BreadthFirstSearchTest, GetOrderedPathToWorksWithDirectedGraph)
{
    GraphForTest graph;
    putConnectionsForTest(graph);
    BfsForTest bfs(graph, 0U);
...
TEST(BreadthFirstSearchTest, GetIsProcessedMapWorksWithDirectedGraph)
{
    GraphForTest graph;
    putConnectionsForTest(graph);
    BfsForTest bfs(graph, 0U);
...
TEST(BreadthFirstSearchTest, ReinitializeStartingFromWorksWithDirectedGraph)
{
    GraphForTest graph;
    putConnectionsForTest(graph);
    BfsForTest bfs(graph, 0U);

    bfs.reinitializeStartingFrom(2U);
    ...
    EXPECT_TRUE(bfs.getShortestPathTo(0U).empty());
    EXPECT_EQ(pathWith1, bfs.getShortestPathTo(1U));
    EXPECT_EQ(pathWith2, bfs.getShortestPathTo(2U));
    EXPECT_EQ(pathWith3, bfs.getShortestPathTo(3U));
    EXPECT_EQ(pathWith4, bfs.getShortestPathTo(4U));
