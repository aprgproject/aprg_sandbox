...
TEST(MonteCarloTest, VerifyingMatrixExample)
{
    ...
    // Of course, we can solve the problem by calculating the product AB again (in O(n3) time using the basic algorithm),
    // but one could hope that verifying the answer would by easier than to calculate it from scratch.
    ...
    MonteCarloAlgorithm verifyingMatrixAlgorithm = [&]()
    {
        ...
        // The time complexity of the algorithm is O(n2), because we can calculate the matrices ABX and CX in O(n2) time.
        // We can calculate the matrix ABX efficiently by using the representation A(BX),
        // so only two multiplications of nxn and nx1 size matrices are needed.
