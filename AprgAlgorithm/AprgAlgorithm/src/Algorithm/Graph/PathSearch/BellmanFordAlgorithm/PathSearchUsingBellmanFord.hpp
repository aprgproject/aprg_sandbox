...
// -> 1) This step initializes distances from the source to all vertices as infinite and distance to the source itself
// as 0.
...
// Proof:
// -> Correctness if G(V, E) has no negative weight cycles, then Bellman-Ford terminate with d[v] = S(s,v) for all v is
// an element of V.
// -> Proof approach: Consider any v in V, by monotonicity and correctness (d[v]>=S(s,v)) only need to show that
// d[v]=S(s,v) at the same time.
...
// because each shortest path can contain at most n-1 edges. In practice, the final distances can usually be found
...
// -> Then, the algorithm always processes the first node in the queue, and when an edge a->b reduces a distance, node b
// is added to the queue. Note: This is implemented above.
