...
private:
    ...
    Vertex getAncestorInternal(Vertex const& vertex, unsigned int const distance) const
    {
        ...
        if(distance > 0) // not zero
        {
            ...
            {
                unsigned int powerOfTwoDistance = BitValueUtilities::get2ToThePowerOf(BitValueUtilities::getLogarithmWithBase2Of(distance));
                result = getAncestorInternal(getAncestorInternal(vertex, distance-powerOfTwoDistance), powerOfTwoDistance);
            }
        }
        else
        {
