...
private:
    ...
    void searchUsingDfsWithDirectedGraph(Vertex const& startVertex)
    {
        ...
        {
            VertexState adjacentVertexState = m_vertexStatesMap.at(adjacentVertex);
            if(SearchType::OneCycle == m_searchType && hasACycle()) // this check is needed to prune all recursion instances once cycle has been detected
            {
                break;
            }
            ...
            else if(VertexState::Processing == adjacentVertexState) // there is a cycle if adjacent vertex is in processing as well
            {
    ...
    void searchUsingDfsWithUndirectedGraph(Vertex const& startVertex, Vertex const& previousVertex)
    {
        ...
        for(Vertex const& adjacentVertex : m_graph.getAdjacentVerticesAt(startVertex))
        {
            ...
            else if(previousVertex != adjacentVertex && VertexState::Processing == adjacentVertexState)  // there is a cycle if adjacent vertex is in processing as well
            {
...
// Algorithm
// The idea is to go through the nodes of the graph and always begin a depth-first search at the current node if it has not been processed yet.
...
// 4) If the graph contains a cycle, we will find this out during the search, because sooner or later we will arrive at a node whose state is 1.
