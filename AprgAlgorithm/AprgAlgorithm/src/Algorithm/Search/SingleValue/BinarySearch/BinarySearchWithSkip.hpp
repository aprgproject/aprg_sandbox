...
private:
    ...
    Index getIndexUsingForwardSkip(Value const& value) const
    {
        ...
        for(Index forwardSkip = size/2; forwardSkip>=1U; forwardSkip/=2U) // forward skip start from half of size, then quarter of size, then eighth of size and so on
        {
            while(result+forwardSkip < size && m_sortedValues.at(result+forwardSkip) < value) // less than condition to avoid redundant traversal on equal values
            {
                result += forwardSkip;
            }
        }
