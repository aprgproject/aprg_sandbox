...
{
public:    // Example for "range query accumulator" is sum queries
    ...
    RangeQueryWithAccumulator(            Values const& valuesToCheck,
            AccumulatorFunction const& accumulator,
            AccumulatorFunction const& inverseAccumulator)
        : m_partialResults()        , m_accumulator(accumulator)
