...
public:
    ...
    RangeQueryForTwoDSegmentTree(
            ValueMatrix const& valueMatrix,
            OneDFunction const& functionObject)
...
private:
    ...
    TwoDFunction getFunctionForSegmentTree2D() const
    {
        return [&](OneDSegmentTree const& tree1, OneDSegmentTree const& tree2)
        {
            ...
            Index estimatedSize = std::min(treeValues1.size()-tree1.getStartOfChildren(), treeValues2.size()-tree2.getStartOfChildren());
            newValues.reserve(estimatedSize);
            for(Index i1=tree1.getStartOfChildren(), i2=tree2.getStartOfChildren();
                i1<treeValues1.size() && i2<treeValues2.size();
                i1++, i2++)
            {
