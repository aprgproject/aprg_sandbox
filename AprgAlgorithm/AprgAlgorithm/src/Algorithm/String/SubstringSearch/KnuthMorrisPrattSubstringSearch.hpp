...
public:
    ...
    KnuthMorrisPrattSubstringSearch(std::string const& substringToMatch)
        ...
        , m_characterToNextStateDfaMatrix(RADIX, substringToMatch.length()) // x coordinate is character and y coordinate is the next state (which is index)
    {
        initialize();
    }
    ...
    Index search(std::string const& stringToSearch)
    {
        ...
        {
            matchIndex = getNextState(matchIndex, stringToSearch.at(searchIndex)); // use DFA to determine next state
        }
...
private:
    ...
    Index getNextState(Index const currentState, unsigned int const triggeringCharacter) const
    {
        return m_characterToNextStateDfaMatrix.getEntry(triggeringCharacter, currentState);
    }

    void setStateTransition(Index const currentState, Index const nextState, unsigned int const triggeringCharacter)
    {
        m_characterToNextStateDfaMatrix.setEntry(triggeringCharacter, currentState, nextState);
    }
    ...
    void initialize()
    {
        ...
        {
            setStateTransition(0, 1, m_substringToMatch.at(0)); // put initial transition of: from first index go to second index (if character is encountered)
            Index matchLength(m_substringToMatch.size());
            ...
            for(Index i=1; i<matchLength; i++)
            {
                ...
                {
                    Index mismatchState(getNextState(stateWithDelayedInput, c)); // assign mismatch state as the "stateWithDelayedInput with inputed c"
                    setStateTransition(i, mismatchState, c); // put transition: if there is a mismatch go back to "stateWithDelayedInput with inputed c" (mismatch state)
                }
                setStateTransition(i, i+1, m_substringToMatch.at(i)); // put transition (overwrite): if match go to the next state
                stateWithDelayedInput = getNextState(stateWithDelayedInput, m_substringToMatch.at(i)); // update state (one tempo delayed)
            }
        }
    }

    std::string m_substringToMatch;
    DfaMatrix m_characterToNextStateDfaMatrix;
};
