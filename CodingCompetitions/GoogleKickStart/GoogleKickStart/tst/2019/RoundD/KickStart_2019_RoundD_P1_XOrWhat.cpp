...
protected:
    ...
    void initialize(Values const& valuesToCheck, Index const suggestedNumberOfBlocks)
    {
        if(!valuesToCheck.empty())
        {
            ...
            Index numberOfBlocks = getMultipleThatIsGreaterOrEqual(static_cast<unsigned int>(valuesToCheck.size()), m_blockSize);
...
RangeQuery::ValuesFunction xorARangeOfValues = [](
        Values::const_iterator itStart, Values::const_iterator itEnd)
{
    return accumulate(itStart+1, itEnd, *itStart, [](Value const value1, Value const value2)
    {
