...
public:
    ...
    History(int const columns, int const rows)
        : m_columnsIntervals(columns)
        , m_rowsIntervals(rows)
    {}
    ...
    Coordinate addSourceAndGetNext(Coordinate const source, Direction const direction)
    {
        ...
        Interval yInterval = addValueToIntervalsAndReturnChangedInterval(m_columnsIntervals[source.first], source.second);
        switch(direction)
        {
...
private:
    ...
    Interval addValueToIntervalsAndReturnChangedInterval(
            Intervals & lineIntervals,
            int const valueToInsert)
    {
        ...
        {
            auto itRight = upper_bound(lineIntervals.begin(), lineIntervals.end(), valueToInsert, [](
                                      int const valueToCheck, Interval const& interval)
            {return valueToCheck < interval.first;});
