...
{
    AlbaLocalPathHandler bitmapDirectory(APRG_BITMAP_FILTERS_BITMAP_DIRECTORY);
    AlbaLocalPathHandler sampleFile(bitmapDirectory.getDirectory()+"BitmapNonPenPixels.bmp");    AlbaLocalPathHandler temporaryFile(bitmapDirectory.getDirectory()+"temporary.bmp");
    temporaryFile.deleteFile();
    sampleFile.copyToNewFile(temporaryFile.getFullPath());
...
{
    AlbaLocalPathHandler bitmapDirectory(APRG_BITMAP_FILTERS_BITMAP_DIRECTORY);
    AlbaLocalPathHandler sampleFile(bitmapDirectory.getDirectory()+"sample.bmp");    AlbaLocalPathHandler temporaryFile(bitmapDirectory.getDirectory()+"temporary.bmp");
    temporaryFile.deleteFile();
    sampleFile.copyToNewFile(temporaryFile.getFullPath());
...
TEST(BitmapFilterTest, DISABLED_SpecialPurpose)
{
    ...
    bitmapFilter.setPenPixelsToOutputCanvas();
    bitmapFilter.saveOutputCanvasToBitmapFile();    temporaryFile.copyToNewFile(temporaryFile.getDirectory()+"BitmapPenPixels.bmp");

    bitmapFilter.clearOutputCanvas();
    bitmapFilter.setBlurredNonPenPixelsToOutputCanvas(5, 0x40);    bitmapFilter.saveOutputCanvasToBitmapFile();
    temporaryFile.copyToNewFile(temporaryFile.getDirectory()+"BitmapNonPenPixels.bmp");
