...
// ---> From cppreference:
// -----> The extern specifier is only allowed in the declarations of variables and functions (except class members or
// function parameters).
...
// -----> All objects declared at namespace scope (including global namespace) have this storage duration, plus those
// declared with static or extern.
// ---> If the data is static, it isn't allocated on the heap, and it will be destructed during the shutdown of the
// process.
// -----> If it is a pointer to the data which is static, e.g.:
// -------> Something* MyClass::aPointer = new Something;
// -----> then like all other dynamically allocated data, it will only be destructed when you delete it. There are two
// frequent solutions:
...
// ---> In multi-threaded environment, these objects might have been destructed while some objects are still accessing
// on it, and then bad things happens.
