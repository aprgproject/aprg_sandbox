...
// -> The static keyword is only used with the declaration of a static member, inside the class definition, but not with
// the definition of that static member.
// -> The declaration inside the class body is not a definition and may declare the member to be of incomplete type
// (other than void),
// ---> including the type in which the member is declared.
// -> However, if the declaration uses constexpr or inline (since C++17) specifier, the member must be declared to have
// complete type.
// -> To refer to a static member m of class T, two forms may be used: qualified name T::m or member access expression
// E.m or E->m,
...
// ---> unless the keyword thread_local is used, in which case there is one such object per thread with thread storage
// duration (since C++11).
...
// (is not a member of unnamed namespace).
// ---> Local classes (classes defined inside functions) and unnamed classes, including member classes of unnamed
...
// ---> it must be initialized with an initializer in which every expression is a constant expression, right inside the
// class definition.
// -> If a const non-inline (since C++17) static data member or a constexpr static data member (since C++11)(until
// C++17) is odr-used,
...
// -> If a static data member is declared constexpr, it is implicitly inline and does not need to be redeclared at
// namespace scope.
// ---> This redeclaration without an initializer (formerly required as shown above) is still permitted, but is
// deprecated.
