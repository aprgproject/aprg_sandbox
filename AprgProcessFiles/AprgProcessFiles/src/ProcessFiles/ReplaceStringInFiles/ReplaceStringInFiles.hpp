...
private:    std::string getNewPrintStreamBasedFromOldPrintFunction(std::string const& printFunction) const;
    ...
    void appendCharacterToResult(std::string & result, bool & isOnStringLiteral, char const c) const;    void appendParameterToResult(std::string & result, bool & isOnStringLiteral, std::string const& parameter) const;
