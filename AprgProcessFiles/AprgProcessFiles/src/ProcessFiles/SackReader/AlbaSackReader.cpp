    ...
    for(unsigned int i=0; i<tokens.size(); i++)
    {
        ...
        if(stringHelper::isIdentifier(token))
        {
            ...
            if(lastIndex >= 1)
            {
                ...
                {
                    tokens.erase(tokens.begin()+recentWords[lastIndex-1].index, tokens.begin()+recentWords[lastIndex].index+1);
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::ClassStateLookingForClassName>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if(stringHelper::isIdentifier(token))
    {
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::ClassStateLookingForOpeningBraceOrSemiColon>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if(";"==token)
    {
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::ClassStateLookingForParameters>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if(stringHelper::isIdentifier(token))
    {
    ...
    {
        transactionData.type.addParameter(AlbaSackReaderParameter(transactionData.firstIdentifier, transactionData.secondIdentifier));
        transactionData.firstIdentifier.clear();
        transactionData.secondIdentifier.clear();
    }
    else if("}"==token)
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::TypedefStateLookingForTypeName>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if("enum"==token)
    {
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::TypedefStateLookingForSecondType>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if(stringHelper::isIdentifier(token))
    {
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::TypedefStateLookingForSemicolon>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if(";"==token)
    {
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::TypedefStructStateLookingForNameOrOpeningBraceOrSecondType>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if("{"==token)
    {
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::TypedefStructStateLookingForParameters>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if(stringHelper::isIdentifier(token))
    {
        transactionData.firstIdentifier=transactionData.secondIdentifier;
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::TypedefStructStateLookingForSecondType>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if(stringHelper::isIdentifier(token))
    {
        transactionData.typeName=token;
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::TypedefEnumStateLookingForOpeningBrace>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if("{"==token)
    {
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::TypedefEnumStateLookingForClosingBrace>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if("}"==token)
    {
...
template <> void AlbaSackReader::analyzeInReaderState<AlbaSackReader::ReaderState::TypedefEnumStateLookingForType>(AlbaSackReader::ReaderTransactionData& transactionData, string const& token)
{
    if(stringHelper::isIdentifier(token))
    {
