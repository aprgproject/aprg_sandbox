...
TEST(AlbaSackReaderTest, TypedefAreRecognized)
{
    ...
    AlbaSackReaderType::TypeNames type2OtherTypes = type2.getOtherTypes();    ASSERT_EQ(0, type1Parameters.size());
...
TEST(AlbaSackReaderTest, TypedefWithStructWithParametersAreRecognized)
{
    ...
    EXPECT_EQ("unsigned long int", myClassParameters[0].getTypeName());
    EXPECT_EQ("integerParameter", myClassParameters[0].getParameterName());    EXPECT_EQ("double", myClassParameters[1].getTypeName());
