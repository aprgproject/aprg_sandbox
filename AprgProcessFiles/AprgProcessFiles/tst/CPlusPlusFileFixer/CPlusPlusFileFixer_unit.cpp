...
TEST(CPlusPlusFileFixerTest, CPlusPlusFileHeadersAreCorrected){
    ...
    testFile << R"(#include <Test1\Test1.hpp>)" << endl;    testFile << R"(#include <Test1\Test1_unit.hpp>)" << endl;
...
{
    CPlusPlusFileFixer fixer;    ofstream testFile(APRG_PROCESS_FILES_TEST_FILE1_TO_READ);
    ASSERT_TRUE(testFile.is_open());
