...
template <typename NumberType> NumberType getSquareRootOfXSquaredPlusYSquaredPlusZSquared(NumberType const x, NumberType const y, NumberType const z);template <typename NumberType> NumberType getSign(NumberType const& value);
...
template <typename NumberType> bool isPerfectSquare(NumberType const& value);template <typename NumberType> bool isPerfectCube(NumberType const& value);
...
AlbaNumber getGreatestCommonFactor(AlbaNumber const& firstNumber, AlbaNumber const& secondNumber);unsigned int getLeastCommonMultiple(unsigned int const firstNumber, unsigned int const secondNumber);
