...
template <typename NumberType> NumberType clampHigherBound(NumberType const value, NumberType const limit);unsigned int getDifferenceFromGreaterMultiple(unsigned int const multiple, unsigned int const number);
