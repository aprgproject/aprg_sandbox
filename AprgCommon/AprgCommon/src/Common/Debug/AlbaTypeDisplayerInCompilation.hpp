...
// NOTES on approach:
// 1) To know the real type, DONT use std::type_info::name, because its required by the standard to change what is displayed.
...
template <typename TypeToDisplay> class DisplayThisType;
...
template <typename TypeInTemplate>
void displayTypeInTemplateByPointerdisplayTypeInTemplateByUniversalReference(TypeInTemplate && parameter)
{
    DisplayThisType<TypeInTemplate> objectWithTypeInTemplate;
    DisplayThisType<decltype(parameter)> objectWithParameterType;
}
