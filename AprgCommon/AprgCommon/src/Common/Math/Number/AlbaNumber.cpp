...
void AlbaNumber::constructBasedFromFractionDetails(NumberType1 const numerator, NumberType2 const denominator)
{
                ...
                static_cast<unsigned int>(getAbsoluteValue(denominator))))
    {
        *this = AlbaNumber(
        ...
        FractionData& fractionDataReference(m_data.fractionData);        fractionDataReference.numerator = fractionDetails.sign * static_cast<int>(fractionDetails.numerator);
...
{
    double result(value);    if(isAlmostAnInteger(value, Configuration::getInstance().getConfigurationDetails().floatAdjustmentTolerance))
    {
        result=round(value);
    }
    return result;
