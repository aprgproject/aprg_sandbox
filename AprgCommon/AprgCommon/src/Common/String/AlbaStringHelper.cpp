...
size_t getNumberOfSubStrings(string_view mainString) {    // A string of length n has n(n+1)/2 substrings.
...
bool isPrefix(string_view mainString, string_view prefix) {    // A prefix is a substring that starts at the beginning of a string.
...
bool isSuffix(string_view mainString, string_view suffix) {    // A suffix is a substring that ends at the end of a string.
...
bool isRotation(string_view mainString, string_view rotation) {    // A rotation can be generated by moving the characters of a string one by one from the beginning to the end (or
    // vice versa). For example, the rotations of ABCD are ABCD, BCDA, CDAB and DABC.
...
void copyBeforeStringAndAfterString(    string_view mainString, string_view stringToSearch, string& beforeString, string& afterString,
