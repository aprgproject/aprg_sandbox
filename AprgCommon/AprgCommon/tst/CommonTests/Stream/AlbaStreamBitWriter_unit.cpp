...
TEST(AlbaStreamBitWriterTest, WriteNumberDataWorks)
{
    AlbaLocalPathHandler testFilePath(APRG_COMMON_TEST_FILE_TO_WRITE);
    {
        ...
        AlbaStreamBitWriter writer(writeTestFile);
        writer.writeNumberData<unsigned int>(AlbaStreamBitEndianType::BigEndian, 0x0A010B0A);
        writer.writeNumberData<unsigned int>(AlbaStreamBitEndianType::LittleEndian, 0x0A010B0A);
    }
    ...
    EXPECT_EQ("\x1\v", fileReader.getLine());
    EXPECT_EQ("", fileReader.getLine());
    EXPECT_EQ("\v\x1", fileReader.getLine());
    EXPECT_TRUE(fileReader.getLine().empty());
    EXPECT_FALSE(fileReader.isNotFinished());
