...
NearestEqualCells::NearestEqualCells(
...
NearestEqualCells::CoordinatePair NearestEqualCells::getNearestEqualPair(Value const value) const
{
    ...
    // If we use Algorithm 1, the running time is O(n^2), because all cells may contain the same letter, and in this case k = n.
    // Also if we use Algorithm 2, the running time is O(n^2), because all cells may have different letters, and in this case n searches are needed.

    // However, we can combine the two algorithms and use different algorithms for different letters depending on how many times each letter appears in the grid.
    // Assume that a letter c appears k times.
    // If k <= sqrt(n), we use Algorithm 1, and if k > sqrt(n), we use Algorithm 2.
    // It turns out that by doing this, the total running time of the algorithm is only O(n*sqrt(n)).
...
NearestEqualCells::CoordinatePair NearestEqualCells::getNearestEqualPairByCheckingAllPairs(
        Value const value) const
{
    ...
    auto twoCoordinatesCombinations(CombinationsGeneration<Coordinates>::generateCombinationsWithLength(coordinates, 2U));
...
NearestEqualCells::CoordinatePair NearestEqualCells::getNearestEqualPairUsingBfs(
        Value const value) const
{
...
NearestEqualCells::Coordinates NearestEqualCells::getCoordinatesWithThisValue(
        Value const value) const
{
    Coordinates result;
    m_valueMatrix.iterateAllThroughYAndThenX([&](unsigned int const x, unsigned int const y)
...
NearestEqualCells::Coordinate NearestEqualCells::getFirstCoordinateUsingBfs(
        Value const value) const
{
    Coordinates coordinates(getCoordinatesWithThisValue(value));
    Bfs bfs(m_coordinateGraph, coordinates);
    return getCoordinateUsingBfs(value, Coordinate{m_valueMatrix.getNumberOfColumns(), m_valueMatrix.getNumberOfRows()}, bfs);
}
...
NearestEqualCells::Coordinate NearestEqualCells::getCoordinateUsingBfs(
        Value const value,
        Coordinate const& firstCoordinate,        Bfs & bfs) const
{
    Coordinate result{};
    bool isFirst(true);
    unsigned int minimumDistance{};
