...
PathSumInGridInRightOrDownWithDiagonalTraversal::Value PathSumInGridInRightOrDownWithDiagonalTraversal::getBestPathSumUsingNaiveRecursion() const
{
    Value pathSum(0);
    ...
    if(!(x==0 && y==0))
    {
        ...
        else
        {
            ...
            result += m_minMaxFunction(
                        m_minMaxFunction(getBestPathSumUsingNaiveRecursion(x-1, y), getBestPathSumUsingNaiveRecursion(x, y-1)),
