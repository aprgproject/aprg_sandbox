...
// -> Traversal uses previous values to compute for a new value// -> The computation of the "needed points to make path" of each cell is:
...
// -> If points[i][j] > 0, then the player could enter (i, j) with points as little as min_Points_on_exit – points[i][j].
// Since he could gain “points[i][j]” points in this cell.
// However, the value of min_Points_on_exit – points[i][j] might drop to 0 or below in this situation.
