...
namespace
{
vector<pair<int, int>> columnsOffsets
{{-1, -1}, {-1, 0}, {-1, 1}, {0, -1}, {0, 0}, {0, 1}, {1, -1}, {1, 0}, {1, 1}};
}

CollectMaximumPointsInAGridUsingTwoTraversals::CollectMaximumPointsInAGridUsingTwoTraversals(
            ...
            for(Index columnLeft(0); columnLeft<m_inputGrid.getNumberOfColumns(); columnLeft++)
            {
                for(Index columnRight(0); columnRight<m_inputGrid.getNumberOfColumns(); columnRight++)
                {
                    ...
                    for(auto const& columnsOffset : columnsOffsets)
                    {
                        ...
                        if(m_inputGrid.isInside(previousColumnLeft, previousRow) && m_inputGrid.isInside(previousColumnRight, previousRow))
                        {
                            Value previousValue = valueGrids.at(previousRow).getEntry(previousColumnLeft, previousColumnRight);
                            if(INVALID_COUNT != previousValue)
                            {
                                entryResult = (entryResult == INVALID_COUNT) ? previousValue : max(entryResult, previousValue);
                            }
                        }
                    }
                    if(INVALID_COUNT != entryResult)
                    {
                        entryResult += (columnLeft==columnRight)
...
CollectMaximumPointsInAGridUsingTwoTraversals::Value CollectMaximumPointsInAGridUsingTwoTraversals::getMaximumPointsUsingNaiveRecursion(
        Index const row,
        Index const columnLeft,
        Index const columnRight) const
{
    Value result(0);
    if(row==m_inputGrid.getNumberOfRows()-1 && !(columnLeft==0 && columnRight==m_inputGrid.getNumberOfColumns()-1)) // if last row did not reach destination
    {
...
CollectMaximumPointsInAGridUsingTwoTraversals::Value CollectMaximumPointsInAGridUsingTwoTraversals::getMaximumPointsUsingMemoizationDP(
    ...
    if(UNUSED_COUNT == result)
    {
        ...
        else
        {
            for(auto const& columnsOffset : columnsOffsets)
            {
                ...
                if(m_inputGrid.isInside(nextColumnLeft, nextRow) && m_inputGrid.isInside(nextColumnRight, nextRow))
                {
                    Value nextValue = getMaximumPointsUsingMemoizationDP(valueGrids, nextRow, nextColumnLeft, nextColumnRight);
                    if(INVALID_COUNT != nextValue)
                    {
                        result = max(result, nextValue);
                    }
