...
MinimumNumberOfJumpsToReachEnd::MinimumNumberOfJumpsToReachEnd(Counts const& sequence)
    : m_sequence(sequence)
{}

MinimumNumberOfJumpsToReachEnd::Count MinimumNumberOfJumpsToReachEnd::getMinimumNumberOfJumpsUsingNaiveRecursion() const
{
...
MinimumNumberOfJumpsToReachEnd::Indices MinimumNumberOfJumpsToReachEnd::getPathOfMinimumNumberOfJumps() const
{
    ...
    if(!m_sequence.empty())
    {
        ...
        for (Index index(1); index<=maxIndexAtNextJump; index++)
        {
            ...
            stepsAtCurrentJump--;            if(stepsAtCurrentJump==0)
            {
                stepsAtCurrentJump = maxIndexAtNextJump-index;
