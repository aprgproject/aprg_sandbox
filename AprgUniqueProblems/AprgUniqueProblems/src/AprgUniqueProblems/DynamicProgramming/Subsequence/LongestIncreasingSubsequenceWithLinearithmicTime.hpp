...
// How can we extend the existing sequences with 8? First of all, can 8 be part of LIS?
// If yes, how? If we want to add 8, it should come after 7 (by replacing 11).
...
// We can replace 11 with 8, as there is potentially best candidate (9) that can extend the new series {2, 3, 7, 8} or {2, 5, 7, 8}.
// Our observation is, assume that the end element of largest sequence is E.
// We can add (replace) current element A[i] to the existing sequence
// if there is an element A[j] (j > i) such that E < A[i] < A[j] or (E > A[i] < A[j] – for replace).
// In the above example, E = 11, A[i] = 8 and A[j] = 9.
// In case of our original array {2, 5, 3}, note that we face same situation when we are adding 3 to increasing sequence {2, 5}.
// I just created two increasing sequences to make explanation simple. Instead of two sequences, 3 can replace 5 in the sequence {2, 5}.
...
// We are adding an element A[i] to these lists.
// We scan the lists (for end elements) in decreasing order of their length.
// We will verify the end elements of all the lists to find a list whose end element is smaller than A[i] (floor value).
