...
LongestPalindromicSubsequence::Count LongestPalindromicSubsequence::getLongestLengthUsingTabularDP() const
{
    ...
    if(!m_string.empty())
    {
        ...
        {            lengthMatrix.setEntry(index2, index2, 1);
            ...
            {
                Count entryResult(0);                if(m_string.at(index1) == m_string.at(index2))
                {
                ...
                else
                {
                    ...
                    entryResult = max(lengthMatrix.getEntry(index1, index2-1),
                                 lengthMatrix.getEntry(index1+1, index2));
                }
