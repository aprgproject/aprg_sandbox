...
MinimumNumberOfSquaresWithSumEqualToN::MinimumNumberOfSquaresWithSumEqualToN(Count const sumOfSquares)
...
MinimumNumberOfSquaresWithSumEqualToN::Count MinimumNumberOfSquaresWithSumEqualToN::getMinimumCountUsingIterativeDP()
    const {
    ...
    for (Count previousSumOfSquares = 1; previousSumOfSquares <= m_sumOfSquares; previousSumOfSquares++) {
        ...
        for (Count base = 1; base * base <= previousSumOfSquares; base++) {
            entryResult =
                min(entryResult,
                    minimumCounts.at(
                        previousSumOfSquares - base * base));  // if difference is zero then its a perfect square
        }
