...
bool TermAnalyzer::isModifiedDueToFunctionDeclaration(Looper const& startLooper)
{
    ...
    array<TermChecker, 3> const termCheckers{TC(TermCheckerType::isCPlusPlusType), TC(T(TermType::WhiteSpace)), TC(T(TermType::Identifier))};
    if(isMultiLineComparisonSatisfiedThenMoveLooper<LooperConnector::None>(compareLooper, termCheckers))
    {
        ...
        if(isBothParenthesesFoundAndMoveLoopers<LooperConnector::WhiteSpaceAndNewLine>(compareLooper, afterOpeningParenthesis, afterClosingParenthesis))
        {
            ...
            if(isSemiColonFoundAndMoveLooper<LooperConnector::WhiteSpaceAndNewLine>(afterSemiColon))
            {
                ...
                fillFunctionSignatureFromFunctionDeclaration(Looper(afterOpeningParenthesis, afterClosingParenthesis-1), cPlusPlusFunctionSignature);
...
bool TermAnalyzer::isModifiedDueToFunctionDefinition(Looper const& startLooper)
{
    TemporaryFindings temporaryFindings(m_findingsBuffer);
    Looper compareLooper(startLooper);
    array<TermChecker, 3> const termCheckers{TC(TermCheckerType::isCPlusPlusType), TC(T(TermType::WhiteSpace)), TC({T(TermType::Identifier), T(TermType::FunctionName)})};
    if(isMultiLineComparisonSatisfiedThenMoveLooper<LooperConnector::None>(compareLooper, termCheckers))
    {
        Looper afterOpeningParenthesis(compareLooper);
        Looper afterClosingParenthesis(compareLooper);
        if(isBothParenthesesFoundAndMoveLoopers<LooperConnector::WhiteSpaceAndNewLine>(compareLooper, afterOpeningParenthesis, afterClosingParenthesis))
        {
            ...
            if(isBothBracesFoundAndMoveLoopers<LooperConnector::WhiteSpaceAndNewLine>(afterClosingParenthesis, afterOpeningBraces, afterClosingBraces))
            {
                ...
                {
                    addFinding(constructFileLocator(__FILE__, __LINE__), "Names does not match in function database", afterOpeningBraces);
                }
                cPlusPlusFunction.addFunctionSignature(cPlusPlusFunctionSignature);
...
void TermAnalyzer::fillFunctionSignatureFromFunctionDeclaration(Looper const& startLooper, CPlusPlusFunctionSignature& cPlusPlusFunctionSignature)
{
    array<TermChecker, 3> const parameterWithName{TC(TermCheckerType::isCPlusPlusType), TC(T(TermType::WhiteSpace)), TC(T(TermType::Identifier))};
    ...
    while (parameterLooper.isNotFinished())
    {
        ...
        {
            if(isMultiLineComparisonSatisfiedThenMoveLooper<LooperConnector::WhiteSpaceAndNewLine>(parameterLooper, parameterWithName))
            {
            ...
            else if(isMultiLineComparisonSatisfiedThenMoveLooper<LooperConnector::WhiteSpaceAndNewLine>(parameterLooper, parameterWithoutName))
            {
                cPlusPlusFunctionSignature.addParameter(parameterLooperCopy.getContentReference().getValueTypeReference(), "");
                isNextParameterExpected = false;
            }
        }
        else if(isMultiLineComparisonSatisfiedThenMoveLooper<LooperConnector::WhiteSpaceAndNewLine>(parameterLooper, separator))
        {
            isNextParameterExpected = true;
        }
        if(parameterLooperCopy.getCurrentIndex() == parameterLooper.getCurrentIndex())
        {
            addFinding(constructFileLocator(__FILE__, __LINE__), "Unprocessed term in function declaration/definition", parameterLooper);
            break;
        }
    };
    ...
    {
        addFinding(constructFileLocator(__FILE__, __LINE__), "Unrecognized function parameters in function declaration/definition", parameterLooper);
    }
}

void TermAnalyzer::fillFunctionSignatureFromFunctionCall(Looper const& startLooper, CPlusPlusFunctionSignature& cPlusPlusFunctionSignature)
{
    ...
    while (parameterLooper.isNotFinished())
    {
        ...
        if(isNextParameterExpected)
        {
            if(isMultiLineComparisonSatisfiedThenMoveLooper<LooperConnector::WhiteSpaceAndNewLine>(parameterLooper, parameter))
            {
    ...
    if(parameterLooper.isNotFinished())
    {
        addFinding(constructFileLocator(__FILE__, __LINE__), "Incorrect function parameters in function call", parameterLooper);
    }
}
