...
CPlusPlusType::CPlusPlusType()
    ...
    , m_isReference(false)
    , m_pointerCount(0)
    , m_typeType(CPlusPlusTypeType::NoType)
    , m_typeName()
{}
...
{
    return out <<"Type: "<<cPlusPlusType.getTypeName()<<
                 " isConst: "<<cPlusPlusType.isConst()<<
                 " PointerCount: "<<cPlusPlusType.getPointerCount()<<
                 " isReference: "<<cPlusPlusType.isReference();
}

string CPlusPlusType::getTypeName() const
{
    return m_typeName;
}
...
int CPlusPlusType::getTemplateInstantiationIndex() const
{
    return m_templateInstantationIndex;
}

int CPlusPlusType::getPointerCount() const
{
    return m_pointerCount;
}

bool CPlusPlusType::isConst() const
{
    return m_isConst;
}

bool CPlusPlusType::isPrimitive() const
{
    return CPlusPlusTypeType::Primitive == m_typeType;
}

bool CPlusPlusType::isClass() const
{
    return CPlusPlusTypeType::Class == m_typeType;
}

bool CPlusPlusType::isTemplateType() const
{
    return CPlusPlusTypeType::TemplateType == m_typeType;
}
...
bool CPlusPlusType::isNotAPointerOrReference() const
{
    return !isPointer() && !isReference();
}

bool CPlusPlusType::isPointer() const
{
    return 0 < m_pointerCount;
}

bool CPlusPlusType::isReference() const
{
...
void CPlusPlusType::incrementPointerCount()
{
    m_pointerCount++;
}
...
void CPlusPlusType::clearReferenceFlag()
{
    m_isReference = false;
}

void CPlusPlusType::setAsReference()
{
    m_isReference = true;
}

void CPlusPlusType::setAsConst()
{
    m_isConst = true;
}
...
void CPlusPlusType::setTypeName(string const& typeName)
{
    m_typeName = typeName;
}

void CPlusPlusType::setTemplateInstantiationIndex(int index)
{
    m_templateInstantationIndex = index;
}
