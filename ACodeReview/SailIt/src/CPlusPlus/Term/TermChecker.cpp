...
TermChecker::TermChecker(TermCheckerType const termCheckerType)
    : m_termCheckerType(termCheckerType)
    , m_term(TermType::Empty)
{}

TermChecker::TermChecker(Term const& term)
    : m_termCheckerType(TermCheckerType::ConcreteTerm)
    , m_term(term)
{}
...
{
    return (out << termChecker.getTermCheckerTypeString()
            << " Term:[" << termChecker.getTerm() << "]");
}
...
bool operator==(Term const& term, TermChecker const& termChecker)
{
    return termChecker.operator==(term);
}
...
TermCheckerType TermChecker::getTermCheckerType() const
{
    return m_termCheckerType;
}

Term TermChecker::getTerm() const
{
    return m_term;
}
