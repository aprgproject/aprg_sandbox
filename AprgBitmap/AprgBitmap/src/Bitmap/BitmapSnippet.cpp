...
void BitmapSnippet::clearAndPutOneColorOnWholeSnippet(unsigned char const colorByte)
{
    ...
    int byteOffsetInXForStart = static_cast<int>(m_configuration.convertPixelsToBytesRoundedToFloor(m_topLeftCorner.getX()));
    int byteOffsetInXForEnd = static_cast<int>(m_configuration.convertPixelsToBytesRoundedToFloor(m_bottomRightCorner.getX()));
...
void BitmapSnippet::loadPixelDataFromFileInConfiguration()
{
    if(m_configuration.isPositionWithinTheBitmap(m_topLeftCorner) && m_configuration.isPositionWithinTheBitmap(m_bottomRightCorner))
    {
        ...
        if(inputStream.is_open())
        {
            int byteOffsetInXForStart = static_cast<int>(m_configuration.convertPixelsToBytesRoundedToFloor(m_topLeftCorner.getX()));
            int byteOffsetInXForEnd = static_cast<int>(m_configuration.convertPixelsToBytesRoundedToFloor(m_bottomRightCorner.getX()));
            int offsetInYForStart = static_cast<int>(m_configuration.getBitmapHeight()) - static_cast<int>(m_topLeftCorner.getY()) - 1;
            int offsetInYForEnd = static_cast<int>(m_configuration.getBitmapHeight()) - static_cast<int>(m_bottomRightCorner.getY()) - 1;
            int numberOfBytesToBeCopiedForX = static_cast<int>(m_configuration.getOneRowSizeInBytesFromBytes(byteOffsetInXForStart, byteOffsetInXForEnd));
...
{
    unsigned int xPartInIndex = m_configuration.convertPixelsToBytesRoundedToFloor(position.getX())-m_configuration.convertPixelsToBytesRoundedToFloor(m_topLeftCorner.getX());
...
uint32_t BitmapSnippet::getPixelAtForPixelInAByte(uint8_t const* reader, unsigned int const index, BitmapXY const position) const
{
    uint32_t result(0);
...
void BitmapSnippet::setPixelAtForPixelInAByte(uint8_t * writer, unsigned int const index, BitmapXY const position, uint32_t const value)
{
    if(index < m_pixelData.getSize())
    {
        ...
        uint32_t retainMask = (m_configuration.getBitMaskForValue() << shiftValue) ^ AlbaBitValueUtilities<uint32_t>::getAllOnes();
