...
public:
    ...
    bool isPositionInside(BitmapXY const position) const;    AprgBitmapConfiguration getConfiguration() const;
    ...
    unsigned int getNumberOfPixelsInSnippet() const;    unsigned int getPixelDataSize() const;
    void clear();
...
private:    unsigned int calculateShiftValue(BitmapXY const position) const;
    ...
    unsigned int getPixelAtForMultipleBytePixels(unsigned char const* reader, unsigned int const index) const;    void setPixelAtForPixelInAByte(unsigned char * writer, unsigned int const index, BitmapXY const position, unsigned int const value);
