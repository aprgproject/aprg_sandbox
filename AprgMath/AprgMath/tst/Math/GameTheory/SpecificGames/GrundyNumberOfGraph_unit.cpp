...
#include <Math/GameTheory/SpecificGames/GrundyNumberOfGraph.hpp>
...
TEST(GrundyNumberOfGraphTest, Example1Works)
{
    DirectedGraphWithAdjacencyMatrix<GrundyNumberOfGraph::Vertex, 10U> graph;
    graph.connect(2U, 1U);
    graph.connect(3U, 2U);
    graph.connect(4U, 2U);
    graph.connect(4U, 3U);
    ...
    graph.connect(9U, 6U);
    graph.connect(9U, 8U);
    GrundyNumberOfGraph gameWithGraph(graph);
