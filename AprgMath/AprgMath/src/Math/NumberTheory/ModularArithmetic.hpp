...
bool canModuloBeDistributedInMultiplication(Number const number1, Number const number2, Number const modulo);bool canModuloBeDistributedInPower(Number const number1, Number const exponent, Number const modulo);
...
bool isEulerTheoremTrue(Number const coPrime1, Number const coPrime2);Number getModularExponentiation(Number const number1, Number const exponent, Number const modulo);
