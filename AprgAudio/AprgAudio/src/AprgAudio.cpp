...
template <class T>
int AprgAudio<T>::getNumSamplesPerChannel() const
{
    if (samples.size() > 0)
...
template <class T>
void AprgAudio<T>::setAudioBufferSize (int numChannels, int numSamples)
{
...
template <class T>
void AprgAudio<T>::setNumSamplesPerChannel (int numSamples)
{
...
AprgAudioFormat AprgAudio<T>::determineAprgAudioFormat (std::vector<unsigned char>& fileData)
{
    ...
    else if (header == "FORM")        return AprgAudioFormat::Aiff;
    else
