...
public:
    ...
    std::string extractAll(std::string const& filePathOfCompressedFile) const;    std::string extractOneFile(std::string const& filePathOfCompressedFile, std::string const& relativePathOfFile) const;
    bool isRecognizedCompressedFile(std::string const& extension) const;
private:
    void extractAllRelevantFilesInThisDirectory(std::string const& directoryPath);    void extractAllRelevantFilesInThisCompressedFile(std::string const& filePathOfCompressedFile);
