...
void TcomToam::handleHwConfigurationMessage(GenericMessage const& genericMessage)
{
    ...
    SpecificStaticMessage<MessageName::TC_HW_CONFIGURATION_MSG> message(convertGenericToSpecificStatic<MessageName::TC_HW_CONFIGURATION_MSG>(genericMessage));    SHwConfigurationMsg const& payload(message.getPayloadReference());
...
void TcomToam::handleLinkStatesMessage(GenericMessage const& genericMessage) const
{
    ...
    SpecificStaticMessage<MessageName::TC_LINK_STATES_MSG> message(convertGenericToSpecificStatic<MessageName::TC_LINK_STATES_MSG>(genericMessage));    sendsLinkStatesResponse();
...
void TcomToam::sendHwConfigurationResponseAck() const
{
    ...
    SHwConfigurationResponseMsg & payload(specificMessage.getPayloadReference());    payload.status = EStatus_NoError;
...
void TcomToam::sendsLinkStatesResponse() const
{
    ...
    send(ComponentName::Oam, convertSpecificStaticToGeneric<MessageName::TC_LINK_STATES_RESP_MSG>(specificMessage));    logNoteOnPreviousMessage("TCOM/TOAM sends Link states response to OAM.");
}

void TcomToam::sendsTcomDeploymentInd() const
{
    SpecificStaticMessage<MessageName::TC_TCOM_DEPLOYMENT_IND_MSG> specificMessage;
    STcomDeploymentIndMsg & payload(specificMessage.getPayloadReference());    logNoteOnComponent("TCOM/TOAM generates wamAddressInd structure base from hardware configuration.");
...
void TcomToam::handleMessageEvent(GenericMessage const& genericMessage)
{
    ...
    case MessageName::TC_HW_CONFIGURATION_MSG:
        ...
        break;    case MessageName::TC_LINK_STATES_MSG:
