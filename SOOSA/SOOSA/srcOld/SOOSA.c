...
void getChebyshevFreqIndex(ChebyshevCriterion* in_cc, int* freqarr, int* indexarr, int start, int finish) {
    ...
    for (i = start; i < finish; i++) {
        LOPPRINT(
            "  FUNCLOOP:getChebyshevFreqIndex[i=%d]->(indexarr[i]=%d|freqarr[i]=%d|mean=%lf)\n", i, indexarr[i],
            freqarr[i], mean);
    ...
    for (i = start; i < finish; i++) {
        LOPPRINT(
            "  FUNCLOOP:getChebyshevFreqIndex[i=%d]->(indexarr[i]=%d|freqarr[i]=%d|stddev=%lf)\n", i, indexarr[i],
            freqarr[i], stddev);
...
int checkIfWithinImageBounds(PairXY in_point, int xSize, int ySize) {
    if (in_point._x < 0 || in_point._y < 0 || in_point._x > xSize || in_point._y > ySize) {
        return 1;
    }
}
...
int getValue(DataDigital* indata, int x, int y) {
    if (x < 0 || y < 0 || x >= (indata->xSizeBytesAllocated * 8) || y >= (indata->ySizeBytesAllocated)) {
        LOPPRINT(
            "  FUNC:getValue(x=%d|y=%d|xalloc:%d|yalloc:%d)->ret=0  |Out of Bounds\n", x + gddx, y + gddy,
            indata->xSizeBytesAllocated, indata->ySizeBytesAllocated);
        return 0;
    }
    if (G_x == x >> 3 && G_y == y) {
        if (((G_value << (x % 8)) & 0x80) == 0) {
            LOPPRINT(
                "  FUNC:getValue(x=%d|y=%d)->(G_x=%d,G_y=%d,G_value=%x) ret=1  |Valid value from global cache\n",
                x + gddx, y + gddy, G_x, G_y, G_value);
            return 1;
        } else {
            LOPPRINT(
                "  FUNC:getValue(x=%d|y=%d)->(G_x=%d,G_y=%d,G_value=%x) ret=0  |Valid value from global cache\n",
                x + gddx, y + gddy, G_x, G_y, G_value);
            return 0;
        };
    }
    ...
    if (((G_value << (x % 8)) & 0x80) == 0) {
        LOPPRINT(
            "  FUNC:getValue(x=%d|y=%d)->(G_x=%d,G_y=%d,G_value=%x) ret=1  |Valid value from data access\n", x + gddx,
            y + gddy, G_x, G_y, G_value);
        return 1;
    } else {
        LOPPRINT(
            "  FUNC:getValue(x=%d|y=%d)->(G_x=%d,G_y=%d,G_value=%x) ret=0  |Valid value from data access\n", x + gddx,
            y + gddy, G_x, G_y, G_value);
        return 0;
    };
}
...
PairXY findIntersection(LineSlopeIntercept verticalLine, LineSlopeIntercept horizontalLine) {
    PairXY outpoint;
    outpoint._x = roundOffSignRobust(
        ((verticalLine.slope * horizontalLine.intercept) + verticalLine.intercept) /
        (1 - (horizontalLine.slope * verticalLine.slope)));
    outpoint._y = roundOffSignRobust(
        ((horizontalLine.slope * verticalLine.intercept) + horizontalLine.intercept) /
        (1 - (horizontalLine.slope * verticalLine.slope)));
    return outpoint;
}
...
void assignMidCircleCriterion(CircleCriterion* circ, CircleCriterion* c1, CircleCriterion* c2, int type) {
    int radius;
    if (type == 1) {
        radius = roundOffSignRobust(((double)c1->radius + c2->radius) / 2);
    } else {
        radius = roundDown(((double)c1->radius + c2->radius) / 2);
    }
    circ->radius = radius;
    circ->diameter = radius * 2;
    circ->radiussquared = pow(radius, 2);
...
int getDataFromBmp(BmpImage* inBmpImage, DataDigital* indata) {
    ...
    if (inBmpImage->setColor == 1) {
        ...
        for (y = ylow; y <= yhigh; y++) {
            ...
            LOPPRINT("  FUNCLOOP:getDataFromBmp[x=%d|y=%d]->(offset=%d|nbor=%d)\n", x, y, offset, nbor);
            fread(ptrC, sizeof(char), xsizebytes, t1fileaddress);
            for (i = 0; i < xsizebytes; i++) {
                ptrC[i] = (ptrC[i]) ^ (0xFF);
            }
        }
    }
    DBGPRINT(
        "  FUNCOUT:getDataFromBmp(LOW(%d,%d) to HIGH(%d,%d))  |Transferred data from BMP to DataDigital.\n",
...
void deAllocData(DataDigital* indata) {    if (indata->status < 2) {
    ...
    indata->buf = NULL;
    indata->xSizeBytesAllocated = 0;
    indata->ySizeBytesAllocated = 0;
    indata->status = 0;
}
...
inline void printDataDigitalProperty(DataDigital* indata) {
    DBGPRINT("\n");
    if (indata->status == 0) {
        INFPRINT("DDINFO: DataDigital Status: IDLE\n");
    } else if (indata->status == 1) {
        INFPRINT("DDINFO: DataDigital Status: POINTS ADDED\n");
    } else if (indata->status == 2) {
        INFPRINT("DDINFO: DataDigital Status: ALLOCATED\n");
    } else if (indata->status == 3) {
        INFPRINT("DDINFO: DataDigital Status: DATA TRANSFERRED, Ready To Use\n");
    }
...
void addPointToDataDigital(BmpImage* inBmpImage, DataDigital* indata, int xp, int yp) {
    DBGPRINT(
        "  FUNC:addPointToDataDigital(xp=%d|yp=%d|xsize=%d|ysize=%d)\n", xp, yp, inBmpImage->xSize, inBmpImage->ySize);
    if (xp < 0) {
        xp = 0;
    }
    if (yp < 0) {
    ...
    if (xp >= inBmpImage->xSize) {
        xp = inBmpImage->xSize - 1;
    }
    if (yp >= inBmpImage->ySize) {
        yp = inBmpImage->ySize - 1;
    }

    if (indata->status == 0) {
        indata->xlow = (xp >> 3) << 3;
        indata->xhigh = (((xp >> 3) + 1) << 3) - 1;
        indata->ylow = yp;
        indata->yhigh = yp;
        indata->status = 1;
    ...
    if (xp < indata->xlow) {
        indata->xlow = (xp >> 3) << 3;
    } else if (xp > indata->xhigh) {
        indata->xhigh = (((xp >> 3) + 1) << 3) - 1;  //=(xrem>0) ? xp+8-xrem : xp;
    }
    if (yp < indata->ylow) {
        indata->ylow = yp;
    } else if (yp > indata->yhigh) {
        indata->yhigh = yp;
    }
    DBGPRINT(
        "  FUNC:addPointToDataDigital(LOW(%d,%d) to HIGH(%d,%d))\n", indata->xlow, indata->ylow, indata->xhigh,
        indata->yhigh);
}

double checkIfShaded(
    DataDigital* indata, PairXY center, CircleCriterion circ, int ques, int col, int choice, PairXY point1,
    PairXY point2) {
    ...
    while (noChange != 0x1111) {
        ...
        if (type == 1) {
            center._x = center._x + 1;
        } else if (type == 2) {
            center._y = center._y + 1;
        } else if (type == 3) {
            center._y = center._y - 1;
        } else if (type == 4) {
            center._x = center._x - 1;
        ...
        if (moved == 0) {
            if (type == 1) {
                noChange = noChange | 0x0001;
            } else if (type == 2) {
                noChange = noChange | 0x0010;
            } else if (type == 3) {
                noChange = noChange | 0x0100;
            } else if (type == 4) {
                noChange = noChange | 0x1000;
            }
        }
        CIRPRINT("CircleCenter:(%d,%d)\n", center._x + gddx, center._y + gddy);
        CIRPRINT(
            "PointLeft:(%d,%d) PointRight:(%d,%d)\n", point1._x + gddx, point1._y + gddy, point2._x + gddx,
            point2._y + gddy);
        CIRPRINT(
            "[type:%d|noChange:%4X|moved:%d]->[ScorePercent=%lf | maxScorePercent=%lf]\n", type, noChange, moved,
            scorePercent, maxScorePercent);
        if (maxScorePercent < scorePercent) {
            maxScorePercent = scorePercent;
        } else {
            moved = 0;
        ...
        if (maxScorePercent >= PERCENTSHADED) {
            break;
        }
    }
#ifdef CIRFLAG
...
int processOneQuestion(DataDigital* indata, PairXY point1, PairXY point2, int ques, int col, CircleCriterion circ) {
    DBGPRINT(
        "  FUNC:processOneQuestion point1(%d,%d) point2(%d,%d)\n", point1._x + gddx, point1._y + gddy, point2._x + gddx,
        point2._y + gddy);
    PairXY temppoint;
    int t1, tch;
    double dtemp;
    ...
    for (t1 = 0; t1 < 5; t1++) {
        ...
        CIRPRINT(
            "DTEMP:%lf (%d + %d)\n", dtemp, point1._x + gddx,
            roundOffSignRobust(((double)point2._x - point1._x) * dtemp));
        ...
        LOPPRINT(
            "  FUNCLOOP:processOneQuestion (x=%d,y=%d,t1=%d)->%lf\n", temppoint._x + gddx, temppoint._y + gddy, t1,
            dtemp);
        if (dtemp > PERCENTSHADED) {
            if (tch == 0) {
                ...
                INFPRINT(
                    "WARN: Multiple answers in ques#=%d column=%d r=%d. Answers: %d and %d\n", ques, col, circ.radius,
                    tch, 5 - t1);
                return 6;
            }
        }
...
int processOneColumn(
    DataDigital* indata, PairXY* QuesArr1, PairXY* QuesArr2, int numQuestionsColumn, int columnNumber) {
    ...
    for (t1 = 0; t1 < numQuestionsColumn; t1++) {
        ...
        LOPPRINT(
            "  FUNCLOOP:processOneColumn point1(x=%d,y=%d) point1(x=%d,y=%d)\n", point1._x, point1._y, point2._x,
            point2._y);
        ...
        if (columnNumber == 1) {
            if (tchoice >= 1 && tchoice <= 5) {
                ColumnAnswers1[t1] = tchoice;
            } else {
                ColumnAnswers1[t1] = 0;
            }
        } else if (columnNumber == 2) {
            if (tchoice >= 1 && tchoice <= 5) {
                ColumnAnswers2[t1] = tchoice;
            } else {
                ColumnAnswers2[t1] = 0;
            }
        }
...
int followLineGetLengthX(DataDigital* indata, PairXY start, PairXY p1) {
    ...
    int state, x, y, pty;
    LOPPRINT(
        "  FUNC:followLineGetLengthX Start(%d|%d) P1(%d|%d) P3(%d|%d)\n", start._x + gddx, start._y + gddy,
        p1._x + gddx, p1._y + gddy);
    x = start._x;
    y = start._y;
    pty = 0;
    state = 0;
    while (1) {
        ...
        if (pty >= PIXELSPERPENLINE) {
        ...
        } else if (getValue(indata, x + p1._x, y + p1._y) == 1) {
            x = x + p1._x;
            y = y + p1._y;
            state = 1;
            pty = pty + 1;
            ...
            while (getValue(indata, x, y - 1) == 1) {
                y = y - 1;
            }
            state = 2;
            LOPPRINT("  FUNCLOOP:followLineGetLengthX->(Find TOP:(%d,%d)\n", x + gddx, y + gddy);
        } else if (state == 2 && getValue(indata, x, y + 1) == 1) {
            y = y + 1;
    ...
    for (lt = 0; lt < numsamplesize; lt++) {  // lineSamples
        ...
        for (xt = in_line[lt_success]._x / 8; xt < xsizebytesallocated; xt++) {  // linebyteloop
            ...
            for (; mt < 8; mt++) {  // linebitloop
                mask = 0x80 >> mt;
                LOPPRINT(
                    "  FUNCLOOPXXX:findLineImageFromLeft[x=%d|y=%d|mt=%d|pt=%d]->dummy=%X|mask%X|bit=%d\n",
                ...
                if ((dummy & mask) == 0x00) {  // black
                    ...
                    pt = pt - ptwhite;
                    in_line[lt_success]._x = roundOffSignRobust(
                        (xt * 8) + mt - 1 - pt + (((double)pt) / 2) - ptwhite);  // minus-1 to get last white
                    LOPPRINT(
                        "  FUNCLOOPXXX:findLineImageFromLeft Successful->[x=%d|y=%d]\n", in_line[lt_success]._x + gddx,
                        in_line[lt_success]._y + gddy);
                    xt = xsizebytesallocated;
                    lt_success++;
                    break;
...
int findLineImageFromRight(
    ...
    for (lt = 0; lt < numsamplesize; lt++) {  // lineSamples
        ...
        mt = in_line[lt_success]._x % 8;
        LOPPRINT(
            "  FUNCLOOP:findLineImageFromRight[lt=%d|x=%d|y=%d]\n", lt, in_line[lt_success]._x + gddx,
            in_line[lt_success]._y + gddy);
        for (xt = in_line[lt_success]._x / 8; xt >= 0; xt--) {  // linebyteloop
            ...
            for (; mt >= 0; mt--) {  // linebitloop
                mask = 0x80 >> mt;
                LOPPRINT(
                    "  FUNCLOOPXXX:findLineImageFromRight[x=%d|y=%d|mt=%d|pt=%d]->bit=%d\n", (xt * 8) + mt + gddx,
                    in_line[lt_success]._y + gddy, mt, pt, (dummy & mask) == 0x00);
                if (pt >= PIXELSPERPENLINE) {
                    xt = -1;
                    break;
                }
                if ((dummy & mask) == 0x00) {  // black
                    ...
                    LOPPRINT(
                        "  FUNCLOOPXXX:findLineImageFromRight Successful->[x=%d|y=%d]\n", in_line[lt_success]._x + gddx,
                        in_line[lt_success]._y + gddy);
                    xt = -1;
                    lt_success++;
                    break;
    ...
    for (lt = 0; lt < lt_success; lt++) {  // lineSamples
        DBGPRINT(
            "  FUNCOUT:findLineImageFromRight[%d]->[x=%d|y=%d]\n", lt, in_line[lt]._x + gddx, in_line[lt]._y + gddy);
    }
#endif
    return lt_success;
}
        ...
        for (iy = in_line[lt_success]._y; iy < ysizebytesallocated; iy++) {  // linebyteloop
            ...
            if ((dummy & mask) == 0x00) {  // black
                ...
                in_line[lt_success]._y =
                    roundOffSignRobust(iy - 1 - pt + (((double)pt) / 2));  // minus-1 to get last white
                pty = 1 + followLineGetLengthX(
                              indata, createXY(in_line[lt_success]._x, in_line[lt_success]._y), createXY(1, 0));
                pty = pty + followLineGetLengthX(
                                indata, createXY(in_line[lt_success]._x, in_line[lt_success]._y), createXY(-1, 0));
                if (pty >= PIXELSPERPENLINE) {
                    LOPPRINT(
                        "  FUNCLOOPXXX:findLineImageFromTop Successful->[x=%d|y=%d]\n", in_line[lt_success]._x + gddx,
                        in_line[lt_success]._y + gddy);
                    lt_success++;
                    break;
                } else {
...
int findLineImageFromBottom(DataDigital* indata, PairXY* in_line, int numsamplesize, PairXY inStart, PairXY inEnd) {
    ...
    for (lt = 0; lt < numsamplesize; lt++) {  // lineSamples
        ...
        pt = 0;
        LOPPRINT(
            "  FUNCLOOP:findLineImageFromBottom[lt=%d|x=%d|y=%d]\n", lt, in_line[lt_success]._x + gddx,
            in_line[lt_success]._y + gddy);
        for (iy = in_line[lt_success]._y; iy >= 0; iy--) {  // linebyteloop
            yt = (iy * xsizebytesallocated);
            dummy = indata->buf[yt + xt];
            LOPPRINT(
                "  FUNCLOOPXXX:findLineImageFromBottom[x=%d,y=%d|pt=%d|dummy=%d|mask=%d]->bit=%d\n",
                in_line[lt_success]._x + gddx, iy + gddy, pt, dummy, mask, (dummy & mask) == 0x00);
            if (pt >= PIXELSPERPENLINE) {
                break;
            }
            if ((dummy & mask) == 0x00) {  // black
                ...
                if (pty >= PIXELSPERPENLINE) {
                    LOPPRINT(
                        "  FUNCLOOPXXX:findLineImageFromBottom Successful->[x=%d|y=%d]\n",
    ...
    for (lt = 0; lt < lt_success; lt++) {  // lineSamples
        DBGPRINT(
            "  FUNCOUT:findLineImageFromBottom[%d]->[x=%d|y=%d]\n", lt, in_line[lt]._x + gddx, in_line[lt]._y + gddy);
    }
#endif
    return lt_success;
}
...
int getSlope(PairXY* in_samples, double* out_slope, int nsize, int type) {  // type=0 is yline, type=1 is xline
    ...
    if (type == 0) {
        for (i = 1; i < nsize; i++) {
            if (in_samples[i]._y == in_samples[i - 1]._y) {
                return 1;
            }
            out_slope[i - 1] =
                ((double)in_samples[i]._x - in_samples[i - 1]._x) / ((double)in_samples[i]._y - in_samples[i - 1]._y);
            LOPPRINT(
                "  FUNCLOOP:getSlope(%d)[ x[i]=%d|y[i]=%d|x[i-1]=%d|y[i-1]=%d ]->slope=%lf\n", i,
                in_samples[i]._x + gddx, in_samples[i]._y + gddy, in_samples[i - 1]._x + gddx,
                in_samples[i - 1]._y + gddy, out_slope[i - 1]);
        }
    } else if (type == 1) {
        for (i = 1; i < nsize; i++) {
            if (in_samples[i]._x == in_samples[i - 1]._x) {
                return 1;
            }
            out_slope[i - 1] =
                ((double)in_samples[i]._y - in_samples[i - 1]._y) / ((double)in_samples[i]._x - in_samples[i - 1]._x);
            LOPPRINT(
                "  FUNCLOOP:getSlope(%d)[ x[i]=%d|y[i]=%d|x[i-1]=%d|y[i-1]=%d ]->slope=%lf\n", i,
                in_samples[i]._x + gddx, in_samples[i]._y + gddy, in_samples[i - 1]._x + gddx,
                in_samples[i - 1]._y + gddy, out_slope[i - 1]);
...
void getBarHeightArr(PairXY* questionsCoor, double* barHeight, int nsize) {
    ...
    for (i = 0; i < nsize; i = i + 2) {
        ...
        LOPPRINT(
            "  FUNCLOOP:getBarHeightArr(j=%d|barHeight=%lf) [%d]->(%ld|%ld) [%d]->(%ld|%ld)\n", j, barHeight[j], i,
            questionsCoor[i]._x + gddx, questionsCoor[i]._y + gddy, i + 1, questionsCoor[i + 1]._x + gddx,
            questionsCoor[i + 1]._y + gddy);
    }
}
double getIntercept(PairXY* in_samples, double slope, int nsize, int type) {
    ...
    if (type == 0) {
        for (i = 0; i < nsize; i++) {
            LOPPRINT(
                "  FUNCLOOP:getIntercept(%d)[x[i]=%d|y[i]=%d]->xintercept=%lf\n", i, in_samples[i]._x + gddx,
                in_samples[i]._y + gddy, intercept);
        ...
        for (i = 0; i < nsize; i++) {
            LOPPRINT(
                "  FUNCLOOP:getIntercept(%d)[x[i]=%d|y[i]=%d]->yintercept=%lf\n", i, in_samples[i]._x + gddx,
                in_samples[i]._y + gddy, intercept);
...
int removeOutliersFromLine(
    PairXY* lineSamples, double* invslope, ChebyshevCriterion* chebycriterion, int numLineSamples, int type) {
    int t1, t2;
    ...
    do {
        ...
        for (t1 = 0; t1 < numLineSamples - 1; t1++) {
            LOPPRINT(
                "  FUNCLOOP:removeOutliersFromLine[t1=%d|(x=%d,y=%d)]->slopeMean:%lf, deviate(k):%lf\n", t1,
                lineSamples[t1]._x + gddx, lineSamples[t1]._y + gddy, invslope[t1],
                getKFromChebyshev(cc, invslope[t1]));
            if (getKFromChebyshev(cc, invslope[t1]) > CHEBYCHEVKVALUE) {
                scoretoremove[t1] = 1;
                scoretoremove[t1 + 1] = 1;
                LOPPRINT(
                    "  FUNCLOOP:removeOutliersFromLine[t1=%d|(x=%d,y=%d)]->deviate(k)=%lf |  delete this, too much "
                    "deviation\n",
                    t1, lineSamples[t1]._x + gddx, lineSamples[t1]._y + gddy, getKFromChebyshev(cc, invslope[t1]));
                LOPPRINT(
                    "  FUNCLOOP:removeOutliersFromLine[t1=%d|(x=%d,y=%d)] |  delete this too, too much deviation\n",
        ...
        for (t1 = 0; t1 < numLineSamples; t1++) {
            if (scoretoremove[t1] == 0) {
                LOPPRINT(
                    "  FUNCLOOP:removeOutliersFromLine(SuccessfulLineSamples)-> (t2=%d|(x=%d|y=%d))\n", t2,
                    lineSamples[t2]._x + gddx, lineSamples[t2]._y + gddy);
                lineSamples[t2] = lineSamples[t1];
                t2++;
            }
        }
    ...
    *chebycriterion = cc;
    ...
    for (t1 = 0; t1 < numLineSamples; t1++) {  // lineSamples
        DBGPRINT(
            "  FUNCOUT:removeOutliersFromLine[%d]->[x=%d|y=%d]\n", t1, lineSamples[t1]._x + gddx,
            lineSamples[t1]._y + gddy);
    }
#endif
    free(scoretoremove);
    scoretoremove = NULL;
...
int removeOutliersFromLineSmart(PairXY* lineSamples, int numLineSamples, int numLineSamplesDesired, int type) {
    ...
    for (; numLineSamplesDesired < numLineSamples; numLineSamples--) {  // Remove Outliers
        ...
        m_least_squares =
            ((total_xy * numLineSamples) - (total_x * total_y)) / ((total_x2 * numLineSamples) - (total_x * total_x));
        b_least_squares = (total_y / numLineSamples) - ((total_x / numLineSamples) * m_least_squares);

        DBGPRINT(
            "  FUNC:removeOutliersFromLineSmart(slope(m_least_squares)=%lf | intercept(b_least_squares)=%lf)\n",
        ...
        for (t1 = 0; t1 < numLineSamples; t1++) {
            if (type == 0) {
                tempDistance =
                    absValDouble((m_least_squares * lineSamples[t1]._y) + b_least_squares - lineSamples[t1]._x);
            } else {
                tempDistance =
                    absValDouble((m_least_squares * lineSamples[t1]._x) + b_least_squares - lineSamples[t1]._y);
            }
            LOPPRINT(
                "  FUNCLOOP:removeOutliersFromLineSmart[t1=%d|(x=%d,y=%d)]->slope:%lf, intercept:%lf, "
                "tempDistance:%lf, maxDistance:%lf\n",
                t1, lineSamples[t1]._x + gddx, lineSamples[t1]._y + gddy, m_least_squares, b_least_squares,
                tempDistance, maxDistance);
            if (tempDistance > maxDistance) {
                // DBGPRINT("  FUNC:removeOutliersFromLineSmart
                // MAXCHANGE[t1=%d|(x=%d,y=%d)]->tempDistance=%lf\n",t1max,lineSamples[t1max]._x+gddx,lineSamples[t1max]._y+gddy,tempDistance);
                maxDistance = tempDistance;
                t1max = t1;
            }
        }
        DBGPRINT(
            "  FUNC:removeOutliersFromLineSmart REMOVED[t1=%d|(x=%d,y=%d)]->maxDistance=%lf\n", t1max,
            lineSamples[t1max]._x + gddx, lineSamples[t1max]._y + gddy, maxDistance);
    ...
    for (t1 = 0; t1 < numLineSamples; t1++) {  // lineSamples
        DBGPRINT(
            "  FUNCOUT:removeOutliersFromLineSmart[%d]->[x=%d|y=%d]\n", t1, lineSamples[t1]._x + gddx,
            lineSamples[t1]._y + gddy);
    }
#endif
    return numLineSamples;
}
void getQFindSamples(
    PairXY* qFindSamples, int numQFindSamples, PairXY start, PairXY finish, LineSlopeIntercept lsi,
    int barheightsamplepixels) {
    int t1, ytemp;
    qFindSamples[0] = start;
    ytemp = start._y;
    DBGPRINT(
        "  FUNC:getQFindSamples start(%d|%d) finish(%d|%d)\n", start._x + gddx, start._y + gddy, finish._x + gddx,
        finish._y + gddy);
    ...
    for (t1 = 1; t1 < numQFindSamples - 1; t1++) {
        ...
        LOPPRINT(
            "  FUNCLOOP:getQFindSamples [t1=%d]->(quesx=%d,quesy=%d)\n", t1, qFindSamples[t1]._x + gddx,
            qFindSamples[t1]._y + gddy);
    }
    qFindSamples[t1] = finish;
    DBGPRINT(
        "  FUNC:getQFindSamples [t1=%d]finish(%d|%d)->(quesx=%d,quesy=%d)  |finish with finishSample\n", t1,
        finish._x + gddx, finish._y + gddy, qFindSamples[t1]._x + gddx, qFindSamples[t1]._y + gddy);
#ifdef DBGFLAG
    for (t1 = 0; t1 < numQFindSamples; t1++) {  // lineSamples
        DBGPRINT(
            "  FUNCOUT:getQFindSamples[%d]->[x=%d|y=%d]\n", t1, qFindSamples[t1]._x + gddx, qFindSamples[t1]._y + gddy);
    }
#endif
}
int getBarWidthCorrectPoint(DataDigital* indata, double invslope, PairXY* io_point, int correctPoint) {
    ...
    if (tvalue == 0) {
        ...
        for (tx = 1; tx < PIXELSPERPENLINE; tx++) {
            ...
            tpoint._x = point._x + tx;
            tpoint._y = point._y + ty;
            tvalue = getValue(indata, tpoint._x, tpoint._y);
            if (tvalue == 1) {
                blackPoint._x = tpoint._x;
                blackPoint._y = tpoint._y;
                width = 1;
                break;
            }
            LOPPRINT(
                "  FUNCLOOP:getBarWidthCorrectPoint FindFirstBlack [tpointx=%d|tpointy=%d]->(tvalue=%X)\n",
    ...
    for (tx = 1; tx < PIXELSPERPENLINE; tx++) {
        ty = roundOffSignRobust(-1 * invslope * (tx));
        tpoint._x = blackPoint._x + tx;
        tpoint._y = blackPoint._y + ty;
        tvalue = getValue(indata, tpoint._x, tpoint._y);
        LOPPRINT(
            "  FUNCLOOP:getBarWidthCorrectPoint [tpointx=%d|tpointy=%d]->(tvalue=%X)->(width=%d)\n", tpoint._x + gddx,
            tpoint._y + gddy, tvalue, width);
        if (tvalue == 1) {
            width = width + whiteWidth + 1;
            whiteWidth = 0;
        } else {
            if (whiteWidth >= PIXELSDIRTWHITEDOT) {
                break;
            }
            whiteWidth++;
        }
    }
    maxx._x = blackPoint._x + tx;
    maxx._y = blackPoint._y + ty;
    whiteWidth = 0;
    for (tx = 1; tx < PIXELSPERPENLINE; tx++) {
        ty = roundOffSignRobust(-1 * invslope * (tx));
        tpoint._x = blackPoint._x - tx;
    ...
    minx._x = blackPoint._x - tx;
    minx._y = blackPoint._y - ty;

    LOPPRINT("  FUNC:getBarWidthCorrectPoint (%d,%d)->(width=%d)\n", io_point->_x + gddx, io_point->_y + gddy, width);
    if (width >= PIXELSPERPENLINE || width <= PIXELSDIRTBLACKDOT) {
        return 0;
    }
    if (correctPoint == 1) {
        (*io_point) = getMidpoint(minx, maxx);
        LOPPRINT(
            "  FUNCOUT:getBarWidthCorrectPoint(minx=%d|miny=%d|maxx=%d|maxy=%d) OUT->(iopointx=%d,iopointy=%d)\n",
...
int barWidthGetCriterion(int* barWidth, int barWidthArrSize, ChebyshevCriterion* ccLine, ChebyshevCriterion* ccBar) {
    ...
    if (indexBarWidth == NULL || freqBarWidth == NULL) {
        if (indexBarWidth != NULL) {
            free(indexBarWidth);
            indexBarWidth = NULL;
        }
        if (freqBarWidth != NULL) {
            free(freqBarWidth);
            freqBarWidth = NULL;
        }
    ...
    for (t1 = 0; t1 < barWidthArrSize; t1++) {
        ...
        for (t2 = 0; t2 < numOfWidths; t2++) {
            if (barWidth[t1] == indexBarWidth[t2]) {
                freqBarWidth[t2]++;
                break;
            }
        }
        if (t2 == numOfWidths) {
            for (t2 = numOfWidths; t2 > 0; t2--) {
                if (barWidth[t1] > indexBarWidth[t2 - 1]) {
                    break;
                }
        ...
        LOPPRINT(
            "  FUNCLOOP:barWidthGetCriterion(t2=%d)->(indexBarWidth=%d|freqBarWidth=%d)\n", t2, indexBarWidth[t2],
            freqBarWidth[t2]);
    }
#ifdef DBGFLAG
    for (t2 = 0; t2 < numOfWidths; t2++) {  // lineSamples
        DBGPRINT(
            "  FUNCOUT:barWidthGetCriterion KmeansArray[%d]->(indexBarWidth=%d|freqBarWidth=%d)\n", t2,
            indexBarWidth[t2], freqBarWidth[t2]);
    }
#endif
    // Kmeans Algorithm
    double mean1, mean2;
    ...
    for (t2 = 1; t2 < numOfWidths - 1; t2++) {
        ...
        for (t1 = 0; t1 < t2; t1++) {
            ...
            LOPPRINT(
                "  FUNCLOOP:barWidthGetCriterion-KMeansAlgorithmMean1(KMeansIndex=%d|t1=%d)->(total1=%lf|num1=%d)\n",
                t2, t1, mean1, tot1);
        }
        mean1 = mean1 / tot1;
        DBGPRINT(
            "  FUNC:barWidthGetCriterion-KMeansAlgorithmMean1(KMeansIndex=%d|t1=%d)->(mean1=%lf|num1=%d)\n", t2, t1,
            mean1, tot1);
        mean2 = 0;
        tot2 = 0;
        for (t1 = t2; t1 < numOfWidths; t1++) {
            ...
            LOPPRINT(
                "  FUNCLOOP:barWidthGetCriterion-KMeansAlgorithmMean2(KMeansIndex=%d|t1=%d)->(total2=%lf|num2=%d)\n",
                t2, t1, mean2, tot2);
        }
        mean2 = mean2 / tot2;
        DBGPRINT(
            "  FUNC:barWidthGetCriterion-KMeansAlgorithmMean2(KMeansIndex=%d|t1=%d)->(mean2=%lf|num2=%d)\n", t2, t1,
            mean2, tot2);
        DBGPRINT(
            "  "
            "FUNC:barWidthGetCriterion-KMeansAlgorithmToAnalyze(KMeansIndex=%d|t1=%d)->(mean1=%lf|num1=%d|mean2=%lf|"
            "num2=%d)\n",
            t2, t1, mean1, tot1, mean2, tot2);
        DBGPRINT(
            "  "
            "FUNC:barWidthGetCriterion-KMeansAlgorithmDeviationFromMean(KMeansIndex=%d|t1=%d)->(mean1=%lf|mean2=%lf|"
            "deviationfrom1=%lf|deviationfrom2=%lf)\n",
    ...
    while (isChanged == 1) {
        ...
        for (t1 = 0; t1 < kmeansindex; t1++) {
            if (freqBarWidth[t1] == 0) {
                continue;
            }
            LOPPRINT(
                "  FUNCLOOP:barWidthGetCriterion-Line(BarWidth=%d|Freq=%d)->(Mean=%lf|StdDev=%lf|deviate(k)=%lf)\n",
            ...
            if (getKFromChebyshev(cc, indexBarWidth[t1]) > CHEBYCHEVKVALUEWIDTH) {
                LOPPRINT(
                    "  FUNCLOOP:barWidthGetCriterion-Line(BarWidth=%d|Freq=%d)->(Mean=%lf|StdDev=%lf|deviate(k)=%lf) | "
                    " delete this, too much deviation\n",
                    indexBarWidth[t1], freqBarWidth[t1], cc.mean, cc.stddev, getKFromChebyshev(cc, indexBarWidth[t1]));
                freqBarWidth[t1] = 0;
                isChanged = 1;
            }
    ...
    *ccLine = cc;
    ...
    while (isChanged == 1) {
        ...
        for (t1 = kmeansindex; t1 < numOfWidths; t1++) {
            ...
            LOPPRINT(
                "  FUNCLOOP:barWidthGetCriterion-Bar (BarWidth=%d|Freq=%d)->(Mean=%lf|StdDev=%lf|deviate(k)=%lf)\n",
                indexBarWidth[t1], freqBarWidth[t1], cc.mean, cc.stddev, getKFromChebyshev(cc, indexBarWidth[t1]));
            if (getKFromChebyshev(cc, indexBarWidth[t1]) > CHEBYCHEVKVALUEWIDTH) {
                LOPPRINT(
                    "  FUNCLOOP:barWidthGetCriterion-Bar (BarWidth=%d|Freq=%d)->(Mean=%lf|StdDev=%lf|deviate(k)=%lf) | "
...
void getQuestions(
    DataDigital* dd, PairXY* in_questions, int* in_numQuestion, PairXY* qFindSamples, int* qFindSamplesType,
    int* barWidth, int numQFindSamples, LineSlopeIntercept tslopeintercept, ChebyshevCriterion ccLine,
    ChebyshevCriterion ccBar) {
    ...
    for (t1 = 0; t1 < numQFindSamples; t1++) {
        if (barWidth[t1] <= PIXELSDIRTBLACKDOT) {
            type = 0;
            DBGPRINT(
                "  FUNC:getQuestions[%d]->(%d,%d):( BARWIDTH=%d "
                "|LineDeviation(k)=%lf|BarDeviation(k)=%lf|AcceptableK=%d) [Dirt] [ptype:%d,type:%d]\n",
                t1, qFindSamples[t1]._x + gddx, qFindSamples[t1]._y + gddy, barWidth[t1],
                getKFromChebyshev(ccLine, barWidth[t1]), getKFromChebyshev(ccBar, barWidth[t1]), CHEBYCHEVKVALUEWIDTH,
                prevtype, type);
        } else if (getKFromChebyshev(ccLine, barWidth[t1]) < CHEBYCHEVKVALUEWIDTH) {
            type = 1;
            DBGPRINT(
                "  FUNC:getQuestions[%d]->(%d,%d):( BARWIDTH=%d "
                "|LineDeviation(k)=%lf|BarDeviation(k)=%lf|AcceptableK=%d) [Line] [ptype:%d,type:%d]\n",
                t1, qFindSamples[t1]._x + gddx, qFindSamples[t1]._y + gddy, barWidth[t1],
                getKFromChebyshev(ccLine, barWidth[t1]), getKFromChebyshev(ccBar, barWidth[t1]), CHEBYCHEVKVALUEWIDTH,
                prevtype, type);
            ...
            DBGPRINT(
                "  FUNC:getQuestions[%d]->(%d,%d):( BARWIDTH=%d "
                "|LineDeviation(k)=%lf|BarDeviation(k)=%lf|AcceptableK=%d) [Bar] [ptype:%d,type:%d]\n",
                t1, qFindSamples[t1]._x + gddx, qFindSamples[t1]._y + gddy, barWidth[t1],
                getKFromChebyshev(ccLine, barWidth[t1]), getKFromChebyshev(ccBar, barWidth[t1]), CHEBYCHEVKVALUEWIDTH,
                prevtype, type);
        } else {
            ...
            DBGPRINT(
                "  FUNC:getQuestions[%d]->(%d,%d):( BARWIDTH=%d "
                "|LineDeviation(k)=%lf|BarDeviation(k)=%lf|AcceptableK=%d) [No Class] [ptype:%d,type:%d]\n",
                t1, qFindSamples[t1]._x + gddx, qFindSamples[t1]._y + gddy, barWidth[t1],
                getKFromChebyshev(ccLine, barWidth[t1]), getKFromChebyshev(ccBar, barWidth[t1]), CHEBYCHEVKVALUEWIDTH,
                prevtype, type);
        }
        if (prevtype == 0 && type == 0 && t1 != 0) {  // both no class case
            ...
            bgnpair2._x = endpair._x - bgnpair._x;
            bgnpair2._y = endpair._y - bgnpair._y;
            limitY = 0;
            t2 = 1;
            while (t2 < bgnpair2._y) {
                ...
                DBGPRINT(
                    "  FUNC:getQuestions-NoClassToNoClass[(%d,%d),(%d,%d)]->temppair(%d,%d)|twidth:%d\n",
                    bgnpair._x + gddx, bgnpair._y + gddy, endpair._x + gddx, endpair._y + gddy, tmppair._x + gddx,
                    tmppair._y + gddy, twidth);
            ...
            while (limitY > 1) {
                ...
                DBGPRINT(
                    "  FUNC:getQuestions-LineToBar[(%d,%d),(%d,%d)]->temppair(%d,%d)|twidth:%d\n", bgnpair._x + gddx,
                    bgnpair._y + gddy, endpair._x + gddx, endpair._y + gddy, tmppair._x + gddx, tmppair._y + gddy,
                    twidth);
                if (twidth <= PIXELSDIRTBLACKDOT) {  // assume line
                    bgnpair2 = tmppair;
                } else if (getKFromChebyshev(ccLine, twidth) < CHEBYCHEVKVALUEWIDTH) {
            ...
            while (endpair._y - bgnpair._y > 1) {
                ...
                twidth = getBarWidthCorrectPoint(dd, tslopeintercept.slope, &tmppair, correctPosition);
                DBGPRINT(
                    "  FUNC:getQuestions-BarToLine[(%d,%d),(%d,%d)]->temppair(%d,%d)|twidth:%d\n", bgnpair._x + gddx,
                    bgnpair._y + gddy, endpair._x + gddx, endpair._y + gddy, tmppair._x + gddx, tmppair._y + gddy,
                    twidth);
                if (twidth <= PIXELSDIRTBLACKDOT) {  // assume line
                    endpair2 = tmppair;
    ...
    if (barHeightArr == NULL) {
        INFPRINT("ERROR: Error in allocating barHeightArr array.\n");
        return;
    }
    ...
    for (t1 = 0; t1 < numImptCoor; t1++) {
        DBGPRINT(
            "  FUNC:getQuestions barHeightArr[%d]|type:%d->(%d,%d)\n", t1, qFindSamplesType[t1],
            qFindSamples[t1]._x + gddx, qFindSamples[t1]._y + gddy);
        if (qFindSamplesType[t1] == 1) {
            ...
            if (BARHEIGHTLOW < tempBarHeight && BARHEIGHTHIGH > tempBarHeight) {
                DBGPRINT(
                    "  FUNC:getQuestions barHeightArr[%d]|type:%d->[numBar:%d](%d-%d=%d)\n", t1, qFindSamplesType[t1],
                    numBarHeightArr, qFindSamples[t1]._y + gddy, prevpair._y + gddy, qFindSamples[t1]._y - prevpair._y);
    ...
    for (t1 = 0; t1 < numImptCoor; t1++) {
        if (qFindSamplesType[t1] == 1) {
            prevpair = qFindSamples[t1];
            DBGPRINT(
                "  FUNC:getQuestions[%d](%d,%d) Type:1 \n", t1, qFindSamples[t1]._x + gddx, qFindSamples[t1]._y + gddy);
            ...
            for (t2 = t1 + 1; t2 < numImptCoor; t2++) {
                ...
                DBGPRINT(
                    "  FUNC:getQuestions[%d]->(height=%lf)<(%lf):(MeanBarHeight=%lf)\n", t2, tempBarHeight, tdouble,
                    cc.mean);
                if (qFindSamplesType[t2] == 2 && BARHEIGHTLOW < tempBarHeight && BARHEIGHTHIGH > tempBarHeight &&
                    absValDouble(tempBarHeight - cc.mean) < tdouble) {
                    DBGPRINT(
                        "  FUNC:getQuestions[%d] Question endpoint candidate "
                        "(%d,%d)(%d,%d)->height(%d):(Mean(k)=%lf)\n",
                        t2, prevpair._x + gddx, prevpair._y + gddy, qFindSamples[t2]._x + gddx,
                        qFindSamples[t2]._y + gddy, tempBarHeight, cc.mean);
                    in_questions[numQuestion]._x = prevpair._x;
                    in_questions[numQuestion]._y = prevpair._y;
                    in_questions[numQuestion + 1]._x = qFindSamples[t2]._x;
                    in_questions[numQuestion + 1]._y = qFindSamples[t2]._y;
                    tdouble = absValDouble(tempBarHeight - cc.mean);
                    t1 = t2;
                    saved = 1;
                }
                if (tempBarHeight > BARHEIGHTHIGH) {
                    break;
                }
            }
            if (saved == 1) {
...
int getQuestionsFromLine(
    DataDigital* in_dd, PairXY* out_questions, int numExpectedQuestions, double* tdoublearr,
    LineSlopeIntercept tslopeintercept, PairXY tcornerup, PairXY tcornerdown, int barheightsamplepixels) {
    ...
    do {
        ...
        if (numQFindSamples < numExpectedQuestions) {
            INFPRINT("ERROR: Number of QFindSamples is not enough.\n");
            fail = 1;
            break;
        }
        qFindSamples = (PairXY*)malloc(numQFindSamples * sizeof(PairXY));
        if (qFindSamples == NULL) {
            INFPRINT("ERROR: Error in allocating qFindSamples array.\n");
            fail = 1;
            break;
        }
        qFindSamplesType = (int*)malloc(numQFindSamples * sizeof(PairXY));
        if (qFindSamplesType == NULL) {
            INFPRINT("ERROR: Error in allocating qFindSamplesType array.\n");
            fail = 1;
            break;
        }
        ...
        if (barWidth == NULL) {
            INFPRINT("ERROR: Error in allocating BarWidth array.\n");
            fail = 1;
            break;
        }
        for (t1 = 0; t1 < numQFindSamples; t1++) {
            barWidth[t1] = getBarWidthCorrectPoint(in_dd, tslopeintercept.slope, &(qFindSamples[t1]), 1);
        }
        ...
        getQuestions(
            in_dd, out_questions, &numQuestionsCoor, qFindSamples, qFindSamplesType, barWidth, numQFindSamples,
            tslopeintercept, ccLine, ccBar);
        free(barWidth);
        barWidth = NULL;
        free(qFindSamples);
        qFindSamples = NULL;
        ...
        if (numQuestionsCoor % 2 == 1) {
            INFPRINT("ERROR: Error in finding questions. Odd number of coordinates for the bars.\n");
            fail = 1;
            break;
        }
        if (numExpectedQuestions * 2 != numQuestionsCoor) {
            INFPRINT("ERROR: Number of questions does not match with expected number\n");
            fail = 1;
            break;
        }
        // for(t1=0;t1<numQuestionsCoor;t1++){getBarWidthCorrectPoint(in_dd, tslopeintercept.slope,
        // &(out_questions[t1]),1);}
        ...
        // if(fail==1){INFPRINT("ERROR: Error in finding questions. The coordinates of the bars are wrong.
        ...
        for (t1 = 0; t1 < numExpectedQuestions; t1++) {
            ...
            // DBGPRINT("  FUNCOUT:getQuestionsFromLine(BarHeight)[%d] -> ( barHeight=%lf Deviation=%lf
            ...
            if (!(BARHEIGHTLOW < tdoublearr[t1] && BARHEIGHTHIGH > tdoublearr[t1])) {
                fail = 1;
                break;
            }
        }
        if (fail == 1) {
            INFPRINT("ERROR: Error in finding questions. Invalid BarHeightFound.\n");
            fail = 1;
            break;
        }
        for (t1 = 0; t1 < numQuestionsCoor; t1++) {
            DBGPRINT(
                "  FUNCOUT:getQuestionsFromLine[%d] -> (x=%d,y=%d)\n", (t1 / 2) + 1, out_questions[t1]._x + gddx,
                out_questions[t1]._y + gddy);
            out_questions[t1]._x = out_questions[t1]._x + in_dd->xlow;
            out_questions[t1]._y = out_questions[t1]._y + in_dd->ylow;
        }
    } while (0);
    if (qFindSamples != NULL) {
        free(qFindSamples);
...
int processOneFile(char* fileName) {
    ...
    while (algoTries < 2) {  // Two Tries only
        ...
        if (algoTries == 0) {
            ...
            addPointToDataDigital(&img, &dd, 0, 0);
            addPointToDataDigital(&img, &dd, xsearchsize, img.ySize - 1);
            if (allocData(&dd) == 1) {
                INFPRINT("ERROR: Error in allocating data for image buffer.\n");
                algoTries++;
                continue;
            }
            if (getDataFromBmp(&img, &dd) == 1) {
                INFPRINT("ERROR: Error in transferring data to image buffer.\n");
                algoTries++;
                continue;
            }
            // printDataDigitalBuffer(&dd);
            numLineSamples = findLineImageFromLeft(
                lineSamples, maxLineSamples, dd.buf, dd.xSizeBytesAllocated, createXY(0, 0),
                createXY(0, img.ySize - 1));
            ...
            if (MINSAMPLESLINE > numLineSamples) {
                INFPRINT(
                    "ERROR: Error in finding the line. Number of samples is not enough (numLineSamples=%d).\n",
                    numLineSamples);
                algoTries++;
                continue;
            }
            if (getSlope(lineSamples, tdoublearr, numLineSamples, 0) == 1) {
                INFPRINT("ERROR: Invalid Slope.\n");
                algoTries++;
                continue;
            }
            ...
            addPointToDataDigital(&img, &dd, img.xSize - 1 - xsearchsize, 0);
            addPointToDataDigital(&img, &dd, img.xSize - 1, img.ySize - 1);
            if (allocData(&dd) == 1) {
                INFPRINT("ERROR: Error in allocating data for image buffer.\n");
                algoTries++;
                continue;
            ...
            // printDataDigitalBuffer(&dd);
            numLineSamples = findLineImageFromRight(
                lineSamples, maxLineSamples, dd.buf, dd.xSizeBytesAllocated, createXY(xsearchsize - 1, 0),
                createXY(xsearchsize - 1, img.ySize - 1));
            DBGPRINT("INFO: BlackSamples=%d\n", numLineSamples);
            numLineSamples = removeOutliersFromLine(lineSamples, tdoublearr, &ccSlope, numLineSamples, 0);
            DBGPRINT("INFO: SucessfulSamples=%d\n", numLineSamples);
            if (MINSAMPLESLINE > numLineSamples) {
            ...
            addPointToDataDigital(&img, &dd, 0, 0);
            addPointToDataDigital(&img, &dd, img.xSize - 1, ysearchsize);
            if (allocData(&dd) == 1) {
                INFPRINT("ERROR: Error in allocating data for image buffer.\n");
                algoTries++;
                continue;
            ...
            // printDataDigitalBuffer(&dd);
            numLineSamples =
                findLineImageFromTop(&dd, lineSamples, maxLineSamples, createXY(0, 0), createXY(img.xSize - 1, 0));
            ...
            addPointToDataDigital(&img, &dd, 0, img.ySize - 1 - ysearchsize);
            addPointToDataDigital(&img, &dd, img.xSize - 1, img.ySize - 1);
            if (allocData(&dd) == 1) {
                INFPRINT("ERROR: Error in allocating data for image buffer.\n");
                algoTries++;
            ...
            // printDataDigitalBuffer(&dd);
            numLineSamples = findLineImageFromBottom(
                &dd, lineSamples, maxLineSamples, createXY(0, ysearchsize - 1),
                createXY(img.xSize - 1, ysearchsize - 1));
            DBGPRINT("INFO: BlackPerpendicularSamples=%d\n", numLineSamples);
            numLineSamples = removeOutliersFromLine(lineSamples, tdoublearr, &ccSlope, numLineSamples, 1);
            DBGPRINT("INFO: SucessfulSamples=%d\n", numLineSamples);
            if (MINSAMPLESLINETOPBOTTOM > numLineSamples) {
            ...
            INFPRINT("INFO: Finding right line. NumOfSamples=%d\n", maxLineSamples);
            numLineSamples = findLineImageFromRight(
                lineSamples, maxLineSamples, dd.buf, dd.xSizeBytesAllocated, createXY(img.xSize - 1, 0),
                createXY(img.xSize - 1, img.ySize - 1));
            DBGPRINT("INFO: BlackSamples=%d\n", numLineSamples);
            numLineSamples = removeOutliersFromLineSmart(lineSamples, numLineSamples, ROBUSTMINSAMPLESLINE, 0);
            DBGPRINT("INFO: SucessfulSamples=%d\n", numLineSamples);
            if (ROBUSTMINSAMPLESLINE > numLineSamples) {
            ...
            INFPRINT("INFO: Finding bottom line. NumOfSamples=%d\n", maxLineSamples);
            numLineSamples = findLineImageFromBottom(
                &dd, lineSamples, maxLineSamples, createXY(0, img.ySize - 1), createXY(img.xSize - 1, img.ySize - 1));
            DBGPRINT("INFO: BlackPerpendicularSamples=%d\n", numLineSamples);
            numLineSamples = removeOutliersFromLineSmart(lineSamples, numLineSamples, ROBUSTMINSAMPLESLINETOPBOTTOM, 1);
            DBGPRINT("INFO: SucessfulSamples=%d\n", numLineSamples);
            if (ROBUSTMINSAMPLESLINETOPBOTTOM > numLineSamples) {
        ...
        if (algoTries == 0) {
            ...
            if (getQuestionsFromLine(
                    &dd, Q1, numQuestionCol1, tdoublearr, templine, temppoint1, temppoint2, barheightsamplepixels) ==
                1) {
            ...
            if (getQuestionsFromLine(
                    &dd, Q4, numQuestionCol2, tdoublearr, templine, temppoint1, temppoint2, barheightsamplepixels) ==
                1) {
            ...
            if (numColumns == 2) {
                ...
                numLineSamples = findLineImageFromRight(
                    lineSamples, maxLineSamples, dd.buf, dd.xSizeBytesAllocated, temppoint1, temppoint2);
                DBGPRINT("INFO: BlackSamples=%d\n", numLineSamples);
                numLineSamples = removeOutliersFromLine(lineSamples, tdoublearr, &ccSlope, numLineSamples, 0);
                DBGPRINT("INFO: SucessfulSamples=%d\n", numLineSamples);
                if (MINSAMPLESLINE > numLineSamples) {
                ...
                temppoint1 = transposePoint(temppoint1, dd.xlow, dd.ylow);
                temppoint2 = findIntersection(centerleftline, bottomline);
                temppoint2 = transposePoint(temppoint2, dd.xlow, dd.ylow);
                templine = transposeLine(centerleftline, 0, dd.xlow, dd.ylow);
                if (getQuestionsFromLine(
                        &dd, Q2, numQuestionCol1, tdoublearr, templine, temppoint1, temppoint2,
                        barheightsamplepixels) == 1) {
                ...
                if (getQuestionsFromLine(
                        &dd, Q3, numQuestionCol2, tdoublearr, templine, temppoint1, temppoint2,
                        barheightsamplepixels) == 1) {
            ...
            // Q1
            if (getQuestionsFromLine(
                    &dd, Q1, numQuestionCol1, tdoublearr, leftline, uplfcorner, dnlfcorner, barheightsamplepixels) ==
                1) {
                INFPRINT("ERROR: Error in finding questions in left line.\n");
                algoTries++;
                continue;
            ...
            // Q4
            if (getQuestionsFromLine(
                    &dd, Q4, numQuestionCol2, tdoublearr, rightline, uprtcorner, dnrtcorner, barheightsamplepixels) ==
                1) {
                INFPRINT("ERROR: Error in finding questions in right line.\n");
                algoTries++;
                continue;
            ...
            if (numColumns == 2) {
                ...
                INFPRINT("INFO: Finding center left line. NumOfSamples=%d\n", maxLineSamples);
                numLineSamples = findLineImageFromRight(
                    lineSamples, maxLineSamples, dd.buf, dd.xSizeBytesAllocated, upcenter, dncenter);
                DBGPRINT("INFO: BlackSamples=%d\n", numLineSamples);
                numLineSamples = removeOutliersFromLineSmart(lineSamples, numLineSamples, ROBUSTMINSAMPLESLINE, 0);
                DBGPRINT("INFO: SucessfulSamples=%d\n", numLineSamples);
                if (ROBUSTMINSAMPLESLINE > numLineSamples) {
                ...
                temppoint2 = findIntersection(centerleftline, bottomline);
                if (getQuestionsFromLine(
                        &dd, Q2, numQuestionCol1, tdoublearr, centerleftline, temppoint1, temppoint2,
                        barheightsamplepixels) == 1) {
                    INFPRINT("ERROR: Error in finding questions in center left line.\n");
                    algoTries++;
                    continue;
                ...
                temppoint2 = findIntersection(centerrightline, bottomline);
                if (getQuestionsFromLine(
                        &dd, Q3, numQuestionCol2, tdoublearr, centerrightline, temppoint1, temppoint2,
                        barheightsamplepixels) == 1) {
                    INFPRINT("ERROR: Error in finding questions in center right line.\n");
                    algoTries++;
                    continue;
        ...
        if (algoTries == 0) {
            if (numColumns == 2) {
                ...
                if (processOneColumn(&dd, Q1, Q2, numQuestionCol1, 1)) {
                    INFPRINT("ERROR: Error in finding number circles.\n");
                    algoTries++;
                    continue;
                }
                deAllocData(&dd);
...
void processDir(char* path) {
    ...
    while (hFind && bContinue) {
        ...
        if (data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            if (!strcmpi(data.cFileName, "thumbs.db")) {
                bContinue = FindNextFile(hFind, &data);
                continue;
            }
            ...
            if (processOneFile(innerPath) == 0) {
                ...
                if (numColumns == 2) {
                    ...
                    for (x = 0; x < numQuestionCol1; x++) {
                        ...
                        if (ColumnAnswers1[x] <= 5 && ColumnAnswers1[x] >= 1) {
                            freqDatabase[i][ColumnAnswers1[x] - 1]++;
                        }
                        i++;
                    }
                    for (x = 0; x < numQuestionCol2; x++) {
                        CSVPRINT(",%d", ColumnAnswers2[x]);
                        if (ColumnAnswers2[x] <= 5 && ColumnAnswers2[x] >= 1) {
                            freqDatabase[i][ColumnAnswers2[x] - 1]++;
                        }
                        i++;
                    }
                } else {
    ...
    while (hFind && bContinue) {
        ...
        if (data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            if (choice == ichoice) {
                ...
                INFPRINT(
                    "Template Information: numColumns=%d numQuestionCol1=%d numQuestionCol2=%d.\n", numColumns,
                    numQuestionCol1, numQuestionCol2);
...
int main(int argc, char* argv[]) {
    ...
    if (numQuestionCol1 > MAXQUESTIONS) {
        INFPRINT(
            "ERROR: Maximum number of questions exceeded. There are %d questions (max=%d).\n", numQuestionCol1,
            MAXQUESTIONS);
        return 1;
    }
    ...
    if (numColumns == 2) {
        for (x = 0; x < numQuestionCol1; x++) {
            CSVPRINT(",Col1_Q%d", x + 1);
        }
        for (x = 0; x < numQuestionCol2; x++) {
            CSVPRINT(",Col2_Q%d", x + 1);
        }
    } else {
        for (x = 0; x < numQuestionCol1; x++) {
            CSVPRINT(",Col1_Q%d", x + 1);
    ...
    while (fgets(str, MAXSTR, basisHtml) != NULL) {
        if (str[0] == '@' && str[1] == 'A' && str[2] == 'A' && str[3] == 'A' && str[4] == '@') {
            ...
            for (x = 0; x < i; x++) {
                ...
                total = freqDatabase[x][0] + freqDatabase[x][1] + freqDatabase[x][2] + freqDatabase[x][3] +
                        freqDatabase[x][4];
                if ((total + 1) % 2 == 0) {
                    median1 = (total + 1) / 2;
                    j = 0;
                ...
                if (total == 0) {
                    ...
                    fprintf(
                        outputHtml, "<td style=\"text-align:center;padding:3px\">%3.1lf%%</td>\n",
                    ...
                    if (median1 % 2 == 0) {
                        ...
                        fprintf(
                            outputHtml, "<td style=\"text-align:center;padding:3px\">%1.1lf</td>\n",
                            ((double)median1) / 2);
                    }
                    fprintf(
                        outputHtml, "<td style=\"text-align:center;padding:3px\">%3.1lf%%</td>\n",
