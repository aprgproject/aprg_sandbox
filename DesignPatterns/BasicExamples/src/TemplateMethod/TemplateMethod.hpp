...
// -> Its important to specify which operations are hooks (may be overriden) and which are abstract operations (must be
// overridden)
// ---> To reuse an abstract class effective, subclass writers must understand which operations are designed for
// overriding.
...
// ---> The template itself should not be overriden; therefore you can make the template method a nonvirtual member
// function.
// -> Minimizing primitive operations
// ---> Minimizing the number of primitive operations that a subclass must override is needed to flesh out the
// algorithm.
