...
public:
    ...
    void buildPartB() override { m_product.makeB("B-X"); }
    void buildPartC() override { m_product.makeC("C-X"); }
    // ...
};

class ConcreteBuilderY : public Builder {
public:
    void buildPartA() override { m_product.makeA("A-Y"); }
    void buildPartB() override { m_product.makeB("B-Y"); }
    void buildPartC() override { m_product.makeC("C-Y"); }
    // ...
};
...
// -----> Because its an abstract interface, all you have to do to change the product internal structure is to define a
// new kind of builder
// -> It isolates code for construction and representation.
// ---> The Builder pattern improves modularity by encapsulating the way a complex object is constructed and
// represented.
// ---> Clients doesn't need to know about the classes that define the product internal structure (such classes are not
// exposed on the builder)
// ---> Each ConcreteBuilder contains all the code to create and assemble a particular kind of product.
// -----> The code is written once and different Directors can reuse it to build Product variants from the same set of
// parts.
// -> It gives you finer control over the construction process.
// ---> Unlike creational pattern that construct product in one shot, the Builder pattern construct the product step by
// step under the director's control.
// ---> Only when the product is finished does the director retrieve it from the builder.
// ---> Hence, the Builder interface reflects the process of constructing the product more than other creational
// patterns.
...
// ---> The Builder class interface must be general enough to allow the construction of product for all kinds of
// concrete builders.
...
// ---> In the common case, the products from concrete builders differ so greatly that there is little to gain from
// having a common product parent class.
// ---> Because the client configures the director with a concrete builder, the client knows handle the products
// properly.
...
// ---> Also in [Builder] returns the product as a final step, but [AbstractFactory] the product gets returned
// immediately.
