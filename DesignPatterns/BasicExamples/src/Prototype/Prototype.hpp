...
// ---> Prototypes let you incorporate a new concrete product class into a system simply by registering a prototypical
// instact with the client.
// ---> That's a bit more flexible thatn other creational pattern, because a client can install and moreve prototypes at
// runtime.
// -> Specifying new objects by varying values.
// ---> Highly dynamic systems let you define new behavior through object composition (by specifying values for an
// object's variable)
// -----> You effectively define new kinds of objects by instantiating existing classes and registering the instance as
...
// ---> For convenience, such applications often let you instantiate complex, user-defined structures, say, to use an
// specific subcircuit again and again.
// -----> We simply add this subcircuit as a prototype to the palette of available circuit elements.
// -----> As long as the composite circuit object implements Clone as a deep copy, circuits with different structure can
// be prototypes.
// -> Reduced subclassing
...
// -----> Instead, an instance of each class is created when its loaded and it registers the instance with a prototype
// manager.
...
// ---> When the number of prototypes in a system isn't fixed (that is, they can be created and destroyed dynamically),
...
// -----> That is, does cloning an object in turn clone its instance variables or do the clone and orginal just share
// the variables.
// ---> A shallow copy is simple and often sufficient and that's what Smalltalk provides by default.
// -----> The default copy constructor in C++ does a member-wise copy, which means pointers will be shared between the
// copy and the original.
// -----> But cloning prototypes with complex structures usually requires a deep copy, because the clone and the
// original must be independent.
...
// -----> then you can use them to provide a default implementation of Clone simply by saing the object and loading it
// back immediately
...
// -> Initializing clones
// ---> While some clients are perfectly happy with the clone as is, other will want to initialize some or all its
// instanal state to values of their choosing.
// -----> You generally can't pass these values in the Clone operation, because their number will vary between classes
// of prototypes.
...
// -----> If not, an "Initialize" operation is needed to take initialization parameters as arguments and sets the clones
...
// clone and return products objects.
