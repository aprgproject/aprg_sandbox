...
bool Polygon<numberOfVertices>::isEquilateral() const {
    ...
    return adjacent_find(lengthOfSides.cbegin(), lengthOfSides.cend(), [](double const length1, double const length2) {               return !isAlmostEqual(length1, length2);
           }) == lengthOfSides.cend();
}
...
bool Polygon<numberOfVertices>::isEquiangular() const {
    ...
    return adjacent_find(               anglesAtVertices.cbegin(), anglesAtVertices.cend(),
...
Lines Polygon<numberOfVertices>::getLines() const {
    Lines lines;
    int sizeMinusOne = static_cast<int>(m_vertices.size()) - 1;    for (int i = 0; i < sizeMinusOne; i++) {
...
AlbaAngles Polygon<numberOfVertices>::getAnglesAtVertices() const {
    ...
    int sizeMinusOne = static_cast<int>(m_vertices.size()) - 1;    anglesAtVertices.emplace_back(
...
void Polygon<numberOfVertices>::getPointsFromVerticesWithoutLastPoint(
    ...
    Line line(firstPoint, secondPoint);    Points pointsFromCurrentLine(line.getPointsWithoutLastPoint(firstPoint, secondPoint, interval));
