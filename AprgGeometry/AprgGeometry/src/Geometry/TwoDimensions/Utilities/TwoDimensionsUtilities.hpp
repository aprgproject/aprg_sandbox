...
bool isPointInsidePolygon(
        Polygon<numberOfVertices> const& polygon,
        Point const& point,
        Point const& pointForRayTesting)
{
    ...
    unsigned int numberOfIntersections = std::count_if(lineSegments.cbegin(), lineSegments.cend(), [&rayLineSegment](LineSegment const& lineSegmentInPolygon)
    {
