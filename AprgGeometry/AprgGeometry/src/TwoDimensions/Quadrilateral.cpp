...
Quadrilateral::StartEndOfXAndY::StartEndOfXAndY(
        double const xStartParameter,
        double const xEndParameter,
        double const yParameter)
    : xStart(xStartParameter)
    , xEnd(xEndParameter)
    , y(yParameter)
{}

void Quadrilateral::traverseArea(
        ...
        TraverseOperation const& traverseOperation) const
{
    ...
    {
        AlbaRange<double> rangeForX(startEndOfXAndY.xStart, startEndOfXAndY.xEnd, interval);
...
Quadrilateral::ListOfStartEndOfXAndY Quadrilateral::getStartEndForXsFor1Points(
...
Quadrilateral::ListOfStartEndOfXAndY Quadrilateral::getStartEndForXsFor2Points(
        Points const& points1,
        Points const& points2,
        double const interval) const
{
    ListOfStartEndOfXAndY result;
    ...
    if(points1Size==1 && points2Size==3)
    {
        ...
        AlbaRange<double> rangeForY(point1.getY(), point21.getY(), interval);
    ...
    else if(points1Size==2 && points2Size==2)
    {
        ...
        AlbaRange<double> rangeForY(point11.getY(), point21.getY(), interval);
        rangeForY.traverse([&](double const y)
        {
    ...
    else if(points1Size==3 && points2Size==1)
    {
        ...
        AlbaRange<double> rangeForY(point11.getY(), point2.getY(), interval);
        rangeForY.traverse([&](double const y)
        {
        ...
        double const interval) const
{
    ...
    if(points1Size==1 && points2Size==1 && points3Size==2)
    {
        ...
        if(point1.getX() < point2.getX())
        {
            ...
            AlbaRange<double> rangeForY(point1.getY(), point2.getY(), interval);
            rangeForY.traverse([&](double const y)
            {
                result.emplace_back(line1To31.calculateXFromY(y), line1To2.calculateXFromY(y), y);
            });
            rangeForY=AlbaRange<double>(point2.getY()+interval, point31.getY(), interval);
            rangeForY.traverse([&](double const y)
            {
    ...
    else if(points1Size==1 && points2Size==2 && points3Size==1)
    {
        ...
        rangeForY=AlbaRange<double>(point21.getY()+interval, point3.getY(), interval);
        rangeForY.traverse([&](double const y)
        {
    ...
    else if(points1Size==2 && points2Size==1 && points3Size==1)
    {
        ...
        if(point2.getX() < point3.getX())
        {
            ...
            rangeForY=AlbaRange<double>(point2.getY()+interval, point3.getY(), interval);
            rangeForY.traverse([&](double const y)
            {
        ...
        double const interval) const
{
    ...
    if(points1.size()==1 && points2.size()==1 && points3.size()==1 && points4.size()==1)
    {
        ...
        if(point2.getX() < point3.getX())
        {
            ...
            rangeForY=AlbaRange<double>(point3.getY()+interval, point4.getY(), interval);
            rangeForY.traverse([&](double const y)
            {
