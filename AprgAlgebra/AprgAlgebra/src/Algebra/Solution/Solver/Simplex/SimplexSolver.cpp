...
bool SimplexSolver::isOptimized() const
{
    return isOptimal(m_simplexTable);
}
...
{
    solveSimplexTable(m_simplexTable);
}
...
void SimplexSolver::initializeSimplexTable(
        ...
        set<unsigned int> const& indicesWithSlackVariables)
{
    m_simplexTable = SimplexMatrix(inputVariableNames.size()+indicesWithSlackVariables.size()+1, constraintsInStandardForm.size()+1);
    ...
    for(unsigned int y=0; y<constraintsInStandardForm.size(); y++)
    {
        ...
        for(Monomial const& monomial : standardFormConstraint.getMonomialsConstReference())
        {
            ...
            {
                m_simplexTable.setEntry(variableNameToIndexMap.at(variableName), y, monomial.getConstantConstReference()); // put variable coefficient
            }
        }
    ...
    for(Monomial const& monomial : objectiveFunction.getMonomialsConstReference())
    {
        ...
        if(variableName.empty())
        {
            m_simplexTable.setEntry(lastX, lastY, monomial.getConstantConstReference()*-1); // put objective function constant
        }
        else
        {
            ...
            {
                m_simplexTable.setEntry(it->second, lastY, monomial.getConstantConstReference()); // put objective function variable coefficient
            }
        }
    }
}
