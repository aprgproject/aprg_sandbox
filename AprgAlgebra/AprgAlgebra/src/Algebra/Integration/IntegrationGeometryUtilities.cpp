...
Term getVolumeUsingOnCrossSectionalArea(
    ...
    return integration.integrateAtDefiniteTerms(
        crossSectionalArea, integralDetails.lowerEnd, integralDetails.higherEnd);
}
...
Term getVolumeUsingOnSolidOfRevolution(
    ...
    // Let the functions f and g be continuous on the closed interval [a, b], and assume that f(x) >= g(x) >= 0 for all
    // x in [a, b]. If V cubic units is the volume of the solid of revolution generated by revolving about the x axis
    // the region bounded by the curves y=f(x) and y=g(x) and the lines x=a and x=b, then
...
Term getVolumeUsingCylindricalShells(Term const& term, DetailsForDefiniteIntegralWithTerms const& integralDetails) {
    ...
    // if S is the solid of revolution obtained by revolving R about the y axis, and if V cubic units is the volume of
    // S, then
...
Term getTotalMassOfARod(Term const& term, DetailsForDefiniteIntegralWithTerms const& integralDetails) {
    ...
    // If p(x) kilograms per meter is the linear density at a point x meters from the origin, where p is continuous on
    // [0, L], then:
...
Term getMomentOfMassOfARod(Term const& term, DetailsForDefiniteIntegralWithTerms const& integralDetails) {
    // A rod of length L meters its left endpoint at the origin and p(x) kilograms per meter is the linear density at a
...
Term getCenterOfMassOfARod(Term const& term, DetailsForDefiniteIntegralWithTerms const& integralDetails) {
    return getMomentOfMassOfARod(term, integralDetails) / getTotalMassOfARod(term, integralDetails);
}
...
Term getRadiusOfGyration(Term const& momentOfInertia, Term const& totalMass) {
    ...
    // as the radial distance to a point which would have a moment of inertia the same as the body's actual distribution
    // of mass, if the total mass of the body were concentrated there. This is applicable to any type of moment of
    // inertia (x axis, y axis, origin)
