        ...
        DetailsForDefiniteIntegralWithTerms const& integralDetails)
{
    ...
    // If S is the solid of revolution obtained by revolving about the x axis the region bounded by the curve y=f(x),    // the x axis and the lines x=a and x=b, and if the V cubic is the volume of S, then
        ...
        DetailsForDefiniteIntegralWithTerms const& integralDetails)
{
    ...
    // If V cubic units is the volume of the solid of revolution generated by revolving about the x axis the region    // bounded by the curves y=f(x) and y=g(x) and the lines x=a and x=b, then
        ...
        DetailsForDefiniteIntegralWithTerms const& integralDetails)
{
    ...
    // Assume that f(x) >= 0 for all x in [a, b].    // If R is the region bounded by the curve y=f(x), the x axis and the lines x=a and x=b,
        ...
        DetailsForDefiniteIntegralWithTerms const& yDetails)
{
    ...
    Term centerOfMassInX = getMomentOfMassOfALaminaWithRespectToYAxis(areaDensityAtPointInXAndY, xDetails, yDetails) / totalMass;    Term centerOfMassInY = getMomentOfMassOfALaminaWithRespectToXAxis(areaDensityAtPointInXAndY, xDetails, yDetails) / totalMass;
        ...
        DetailsForDefiniteIntegralWithTerms const& yDetails)
{
    ...
    Term derivativeInY(getPartialDerivative(z, yDetails.variableName));    Term termInsideSquareRoot((derivativeInX^Term(2)) + (derivativeInY^Term(2)) + 1);
