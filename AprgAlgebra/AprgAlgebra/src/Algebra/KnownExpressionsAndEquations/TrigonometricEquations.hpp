...
Term getTangentOfSumOrDifferenceOfTwoTerms(Term const& term1, Operator const& operatorObject, Term const& term2);Term getSineOfDoubledValue(Term const& term);
Term getCosineOfDoubledValue(Term const& term);
Term getTangentOfDoubledValue(Term const& term);
Term getSineOfHalvedValue(Term const& term, bool const isPositiveRoot);Term getCosineOfHalvedValue(Term const& term, bool const isPositiveRoot);
