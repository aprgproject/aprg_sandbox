...
AxisRotation::AxisRotation(
        ...
        string const& yName,
        double const angle,
        bool const doRotationBack)
{
    initializeSubstitution(xName, yName, angle, doRotationBack);
}

Term AxisRotation::rotateTermByAngle(Term const& term)
{
    return m_substitution.performSubstitutionTo(term);
}
        ...
        bool const doRotationBack)
{
    ...
    if(doRotationBack)
    {
        newX = Term(oldX*cosTheta - oldY*sinTheta);
        newY = Term(oldX*sinTheta + oldY*cosTheta);
    }
    else
    {
        newX = Term(oldX*cosTheta + oldY*sinTheta);
