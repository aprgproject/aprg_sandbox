...
TEST(CommonFunctionLibraryTest, LogarithmsCanSimplifiedToLoweringOperatorValue) {
    ...
    Term quotientLogarithm(ln(AlbaNumber(5) / 7));    Term exponentLogarithm(ln(AlbaNumber(5) ^ 7));
...
TEST(CommonFunctionLibraryTest, HarmonicNumberFunctionHasAnUpperBoundOfLog2NPlusOne) {
    ...
    Term actualHarmonicNumber(harmonicNumber(sampleValue));    Term upperBoundValue(getLogarithm(2, sampleValue.getDouble()) + 1);  // upper bound: lg(N)+1
