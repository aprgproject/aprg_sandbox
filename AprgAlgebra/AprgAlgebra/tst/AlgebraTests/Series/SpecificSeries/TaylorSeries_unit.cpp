...
TEST(TaylorSeriesTest, GetFormulaForTaylorSeriesWorks)
{
    ...
    string stringToExpect("((e)^x)");    EXPECT_EQ(stringToExpect, series.getFormulaForTaylorSeries().getDisplayableString());
}
...
TEST(TaylorSeriesTest, GetValueAtIndexWorks)
{
    ...
    string stringToExpect1("1");    string stringToExpect2("(e)[x]");
    ...
    EXPECT_EQ(stringToExpect2, series.getValueAtIndex(1, 1).getDisplayableString());    EXPECT_EQ(stringToExpect3, series.getValueAtIndex(2, 2).getDisplayableString());
}
...
TEST(TaylorSeriesTest, GetValueForMaclaurinSeriesAtIndexWorks)
{
    ...
    string stringToExpect1("1");    string stringToExpect2("(1[x] + 1)");
    string stringToExpect3("((1/2)[x^2] + 1[x] + 1)");
    string stringToExpect4("((1/6)[x^3] + (1/2)[x^2] + 1[x] + 1)");
    EXPECT_EQ(stringToExpect1, series.getValueForMaclaurinSeriesAtIndex(0).getDisplayableString());    EXPECT_EQ(stringToExpect2, series.getValueForMaclaurinSeriesAtIndex(1).getDisplayableString());
