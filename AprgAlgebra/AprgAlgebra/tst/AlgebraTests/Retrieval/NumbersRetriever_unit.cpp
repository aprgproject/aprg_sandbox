...
TEST(NumbersRetrieverTest, RetrieveFromTermWorks)
{
    ...
    retriever.retrieveFromTerm(Monomial(34, {{"x", 5}, {"y", 6}}));    retriever.retrieveFromTerm(Polynomial{Monomial(516, {{"x", 7}}), Monomial(643, {{"y", 8}})});
...
TEST(NumbersRetrieverTest, RetrieveFromFunctionWorks)
{
    ...
    AlbaNumbersSet const& numbersSet(retriever.getSavedData());    ASSERT_EQ(3U, numbersSet.size());
    AlbaNumbersSet::const_iterator it = numbersSet.cbegin();
